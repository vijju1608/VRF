...
namespace JCHVRF.MyPipingBLL
{
    ...
    {
        OK = 0,   //通过
        LINK_LENGTH = -1,  //每段管长不能为0
        WARN_ACTLENGTH = -2,  //最长管实际管长超出范围
        EQLENGTH = -3,  //最长等效管长超出范围
        FIRSTLENGTH = -4,  //第一分歧管到最远端的最长等效管长超出限制
        LENGTHFACTOR = -5,  //
        TOTALLIQUIDLENGTH = -6,  //
        MKTOINDOORLENGTH = -7,  //
        MKTOINDOORLENGTH1 = -8,  //
        ...
        MAX_HIGHDIFF_INDOOR = -24,  //Height Difference between Indoor units

        MAX_CHBOXHIGHDIFF = -25,        //Height Difference between CH-boxes
        MAX_MULTICHBOXHIGHDIFF = -26,   //Height Difference between Indoor Units using the Same Branch of CH-Box
        MAX_CHBOX_INDOORHIGHDIFF = -27, //Height Difference between CH-Box and Indoor Units

        INDOORLENGTH_HIGHDIFF = -28,    //室内机到室外机的管长 小于室内机设置的高度差
        CHBOXLENGTH_HIGHDIFF = -29,      //CHBOX到室外机的管长 小于CHBox设置的高度差
        CHBOX_INDOORLENGTH_HIGHDIFF = -30,//室外机到CHBOX的管长 小于室内机到CHBOX的高度差

        PIPING_CHTOINDOORTOTALLENGTH = -31, // CH-Box到每台室内机的总管长超出限制
        PIPING_LENGTH_HEIGHT_DIFF = -32,      //验证实际管长与高度差

        MIN_DISTANCE_BETWEEN_MULTI_KITS = -33,  //minimum distances between multikits is 0.5 meters
        ODU_PIPE_LENGTH_LIMITS = -34,  //室外机组或连接套件的管道长度超出限制。

        MAX_BIG_IDU_OF_MUlTI_CHBOX = -35, //每个multiple CH-Box最多只能连接2个8HP/10HP的IDU。

        MANUAL_PIPING_INDOORS_MISSING = -36, //手工配管图，并非所有室内机都已经用到。
        MANUAL_PIPING_REDUNDANT_UNITS = -37,  //手工配管图，缺少子节点。
    ...
    public class PipingBLL
    {
        ...
        #region 为了将frmMain的Piping相关代码移植过来，增加的全局变量 add on 20170506 by Shen Junjie
        ...
        //public PipingBLL(Project thisProj)
        //    : this()
        //{
        ...
        /// <summary>
        /// 为了将frmMain的Piping相关代码移植过来，增加的构造函数 add on 20170506 by Shen Junjie
        /// </summary>
        ...
        public static string GetFactoryCode(string modelFull)
        {
            ...
            if (modelFull == "") return "";
            return modelFull.Substring(modelFull.Length - 1, 1);  //第十四位表示工厂代码
        }
        ...
        /// 获取CH box对象，增加室内机数量限制 modify on 20160529 by Yunxiao Lin
        ...
        {
            return _dal.GetPipingNodeIndElement(unitType, isHitachi);
        }
        ...
        //Pipe diameter increase should be before single CH Box, not after.modified by Shen Junjie on 2018/7/27
        /// <summary>
        /// 如果CH Box后面连接的室内机大于4个，则前面和后面的管道增大一号
        /// </summary>
        /// <returns></returns>
        ...
        private void CheckIndoorNumberConnectedCHBox(List<string> listPipeSize, Node node, bool needSizeUp)
        {
            ...
            else if (node is MyNodeCH)
            {
                ...
                //只适用于xxxxN2的型号 add by Shen Junjie on 2018/07/27
        ...
        /// <returns></returns>
        public SizeF GetAddFlowSize(AddFlow addFlowItem)
        {
            ...
            foreach (Node node in addFlowItem.Nodes)
            {
                if (node is MyNodeIn)
                {
                    float width = node.Location.X + node.Size.Width + 10;
                    float height = node.Location.Y + node.Size.Height + 10;
        ...
        public bool CheckManualPipingComplete(Node node)
        {
            if (node == null)
                return false;
            ...
            if (node is MyNodeOut)
            {
                ...
                {
                    nodeOut.DrawColor = utilPiping.colorUncompleted;
                    return false;
                }
                else
                {
                    nodeOut.DrawColor = utilPiping.colorCompleted;
                    return CheckManualPipingComplete(nodeOut.ChildNode);
                }
            }
            else if (node is MyNodeYP)
            {
                ...
                if (nodeYP.IsCP)
                {
                    if (nodeYP.ChildCount > 0)
                    {
                        nodeYP.FillColor = utilPiping.colorYP;
                    }
                    else
                    {
                        nodeYP.FillColor = utilPiping.colorUncompleted;
                        return false;
                    }
                }
                else
                ...
                {
                    Node child = nodeYP.ChildNodes[i];
                    if (child != null)
                    {
                        isComplete = CheckManualPipingComplete(child) && isComplete;
                    }
                }
                return isComplete;
            }
            else if (node is MyNodeCH)
            {
                ...
                {
                    nodeCH.DrawColor = utilPiping.colorUncompleted;
                    return false;
                }
                else
                {
                    nodeCH.DrawColor = utilPiping.colorCompleted;
                    return CheckManualPipingComplete(nodeCH.ChildNode);
                }
            }
            else if (node is MyNodeMultiCH)
            {
                MyNodeMultiCH nodeMCH = node as MyNodeMultiCH;
                if (nodeMCH.ChildNodes.Count > 0)
                {
                    nodeMCH.DrawColor = utilPiping.colorCompleted;
                }
                else
                {
                    nodeMCH.DrawColor = utilPiping.colorUncompleted;
                    return false;
                }
                bool isComplete = true;
                for (int i = 0; i < nodeMCH.ChildNodes.Count; ++i)
                {
                    isComplete = CheckManualPipingComplete(nodeMCH.ChildNodes[i]) && isComplete;
                }
                return isComplete;
            }
            else if (node is MyNodeIn)
            {
                ...
                {
                    nodeIn.DrawColor = utilPiping.colorUncompleted;
                    return false;
                }
                else
                {
                    nodeIn.DrawColor = utilPiping.colorCompleted;
                }
            }
            else
            {
                node.DrawColor = utilPiping.colorUncompleted;
                return false;
            }
            return true;
        }
        ...
        public PipingErrors ValidateManualPiping(SystemVRF sysItem)
        {
            MyNodeOut nodeOut = sysItem.MyPipingNodeOut;

            if (!CheckManualPipingComplete(nodeOut))
            {
                return PipingErrors.MANUAL_PIPING_REDUNDANT_UNITS;
            }
            ...
            PipingBLL.EachNode(nodeOut, (node) =>
            {
                ...
                {
                    MyNodeIn nodeIn = node as MyNodeIn;
                    RoomIndoor indoorFound = indoors.Find(p => p.IndoorNO == nodeIn.RoomIndooItem.IndoorNO);
                    if (indoorFound != null)
                    {
                        indoors.Remove(indoorFound);
                    }
                    else
                    {
                        notFound = true;
            ...
            if (indoors.Count != 0 || notFound)  //室内机没有全部在图上，或图上的室内机不在室内机列表
            {
                return PipingErrors.MANUAL_PIPING_INDOORS_MISSING;
            }
        ...
        /// -7  -- 末端分歧管到室内机最大管长
        /// </returns>
        public PipingErrors ValidatePipeLength(SystemVRF sysItem, AddFlow addFlowItem)
        {
            ...
            {
                errorType = ValLinkLengthMin(sysItem, addFlowItem); //检验每段管长不能为0
            }
            if (errorType == PipingErrors.OK)
            {
                ...
                if (node1 is MyNodeMultiCH)
                {
                    ...
                    EachNode(node1, node2 =>
                    {
                        if (node2 is MyNodeIn)
                        {
                            ...
                            //每个multiple CH-Box最多只能连接2个7HP/8HP/10HP的IDU。add by Shen Junjie on 2018/8/17
                            //适用所有multi CH Box 不限区域 modified by Shen Junjie on 2018/9/28
                            if (horsePower == 7 || horsePower == 8 || horsePower == 10)
                            {
                                count++;
                            }
                        }
                    ...
                    if (count > 2)
                    {
                        ...
                        //适用所有multi CH Box 不限区域 modified by Shen Junjie on 2018/9/28
                        err = PipingErrors.MAX_BIG_IDU_OF_MUlTI_CHBOX;
                    }
                    return true;  //结束遍历。
                }
                return false;
        ...
        /// Height Difference between Outdoor Units and Indoor Units，室外单元和室内单元之间的高度差异(Upper) add on 20180502 by xyj
        /// <summary>
        ...
        /// Height Difference between Outdoor Units and Indoor Units，室外单元和室内单元之间的高度差异(Lower) add on 20180502 by xyj
        /// <summary>
        ...
        /// Height Difference between CH-Box and Indoor Units，CH-Box和室内单元之间的高度差异 add on 20180620 by xyj
        /// <summary>
        /// CH-Box和室内单元之间的高度差异
        /// </summary>
        ...
        /// Height Difference between Indoor Units using the Same Branch of CH-Box，使用CH-Box的同一分支的室内单元之间的高度差异 add on 20180620 by xyj
        /// <summary>
        ...
        //    {
        //        link.DrawColor = linkColor;
        //    }
        //}

        public void SetDefaultColor(AddFlow addFlowPiping)
        {
            foreach (Node node in addFlowPiping.Nodes)
            {
                if (node is MyNodeOut)
                {
                    node.DrawColor = utilPiping.colorTransparent;
                }
                if (node is MyNode)
                {
                    utilPiping.setItemDefault(node);
                }
            }
        }
        ...
        /// </summary>
        /// <param name="minLength"></param>
        private PipingErrors ValLinkLengthMin(SystemVRF sysItem, AddFlow addFlowPiping)
        {
            SetDefaultColor(addFlowPiping);
            MyNodeOut nodeOut = sysItem.MyPipingNodeOut;
            //add by Shen Junjie on 2018/8/2
            if (nodeOut != null && nodeOut.UnitCount > 1)
            {
                if (nodeOut.PipeLengthes == null
                    //室外机组台数可能变化，管子数量会跟随变化 add by Shen Junjie on 2018/12/21
                    || nodeOut.PipeLengthes.Length < nodeOut.UnitCount * 2 - 2)
                {
                    nodeOut.DrawColor = utilPiping.colorWarning;
                ...
                if (nodeOut.OutLinks.Count > 0)
                {
                    Link lk = nodeOut.OutLinks[0];
                    MyLink link = (MyLink)lk;
                    if (link.Length < 0.5)
                    {
                        //Org是connection kit需要判断不能小于0.5m
                        link.DrawColor = utilPiping.colorWarning;
        ...
        // add by Shen Junjie on 2018/12/16
        /// <summary>
        /// 验证第一分歧管一个分支下的最近的室内机和最远的室内机的管长差是否大于40米
        /// </summary>
        ...
        /// <summary>
        /// 查找第一分歧管一个分支下的最近的室内机和最远的室内机
        /// </summary>
        /// <param name="node">分支起点</param>
        /// <param name="furthestIU">找到的最远的室内机</param>
        /// <param name="closestIU">找到的最近的室内机</param>
        /// <param name="maxLength">第一分歧管到最远的室内机的管长</param>
        /// <param name="minLength">第一分歧管到最近的室内机的管长</param>
        ...
        public PipingErrors ValMaxRealLength(SystemVRF curSystemItem, AddFlow addFlowPiping)
        {
            ...
            foreach (Node node in addFlowPiping.Nodes)
            {
                if (node is MyNodeIn)
                {
                    ...
                    if (curSystemItem.SysType == SystemType.OnlyFreshAir && curSystemItem.MaxPipeLengthwithFA > 0 && tempRealLength > curSystemItem.MaxPipeLengthwithFA)
                    {
        ...
        //绘制错误节点
        public void DrawNodeErrorLinks(Node node, SystemVRF curSystemItem)
        {
            if (node == null) return;
            ...
            {
                MyNodeOut nodeOut = node as MyNodeOut;
                DrawNodeErrorLinks(nodeOut.ChildNode, curSystemItem);
            }
            else if (node is MyNodeCH)
            {
                MyNodeCH nodeCH = node as MyNodeCH;
                DrawNodeErrorLinks(nodeCH.ChildNode, curSystemItem);
            }
            else if (node is MyNodeMultiCH)
            {
                MyNodeMultiCH nodeMCH = node as MyNodeMultiCH;
                foreach (Node child in nodeMCH.ChildNodes)
                {
                    DrawNodeErrorLinks(child, curSystemItem);
                }
            }
            else if (node is MyNodeYP)
            {
        ...
        /// <summary>
        /// 室内机到CHBox之间的管长不能小于室内机和CHBox的高度差
        /// </summary>
        /// <param name="nodeIn"></param>
        /// <param name="chHighDiff"></param>
        /// <param name="ch_ODU_Length"></param>
        ...
        private double CHBox_IndoorLength(MyNodeIn nodeIn, ref double Inddiff)
        {
            ...
            if (nodeIn == null)
                return length;
            if (nodeIn.MyInLinks != null || nodeIn.InLinks.Count == 0)
            {
                Link lk = nodeIn.InLinks[0];
                if (lk is MyLink)
                {
                    MyLink link = (MyLink)lk;
                    length = link.Length;
            ...
            if (nodeIn.RoomIndooItem.PositionType == PipingPositionType.Lower.ToString())
            {
                Inddiff = -nodeIn.RoomIndooItem.HeightDiff;
            }
            return length;
        ...
        public bool ValTotalLquidLength(SystemVRF sysItem)
        {
            ...
            Outdoor outItem = sysItem.OutdoorItem;
        ...
        private void GetPipeLengthMaxMKToIndoor(Node parent, Node node, bool underCHBox, out Node nodeStart, out Node nodeEnd, out double L3)
        {
            ...
            else if (node is MyNodeYP)
            {
                ...
                for (int i = 0; i < nodeYP.MaxCount; i++)
                {
                    ...
                    {
                        //L3不再区分是不是从First branch开始。就是说First branch开始的也是L3。 modified by Shen Junjie on 20181022
                        //if (nodeStart1 == null && !nodeYP.IsFirstYP)  
                        if (nodeStart1 == null)
                        {
                            nodeStart1 = nodeYP;
                        }
        ...
        /// modified by Shen Junjie on 2018/10/16
        /// <summary>
        /// 计算并验证CH-Box下端所有管长总和，本函数只能在配管计算得到CH-Box型号之后执行
        /// </summary>
        public PipingErrors ValCHToIndoorMaxTotalLength(SystemVRF curSystemItem, Node node)
        {
            if (node == null) return PipingErrors.OK;
            ...
            {
                MyNodeOut nodeOut = node as MyNodeOut;
                return ValCHToIndoorMaxTotalLength(curSystemItem, nodeOut.ChildNode);
            }
            else if (node is MyNodeYP)
            {
                MyNodeYP nodeYP = node as MyNodeYP;
                foreach (Node child in nodeYP.ChildNodes)
                {
                    PipingErrors childResult = ValCHToIndoorMaxTotalLength(curSystemItem, child);
                    if (childResult != PipingErrors.OK)
                    {
                        return childResult;
                    }
            ...
            {
                MyNodeCH nodeCH = node as MyNodeCH;
                if (nodeCH.MaxTotalCHIndoorPipeLength > 0)
                {
                    //double childPipeLength = getTotalToIndoorLength(nodeCH.ChildNode);  //CHBOX - ... -INDOOR 的实际管长
                    double childPipeLength = nodeCH.ActualTotalCHIndoorPipeLength;
                    if (childPipeLength > nodeCH.MaxTotalCHIndoorPipeLength)
                    {
                        DrawNodeErrorLinks(nodeCH.ChildNode, curSystemItem);
                        setLinkPathColor(nodeCH, utilPiping.colorDefault, curSystemItem.OutdoorItem.ProductType);
                        TempActualLength = childPipeLength;
                        TempMaxLength = nodeCH.MaxTotalCHIndoorPipeLength;
                        return PipingErrors.PIPING_CHTOINDOORTOTALLENGTH;//CH-Box到每台室内机的总管长超出限制
                    }
                }

                return ValCHToIndoorMaxTotalLength(curSystemItem, nodeCH.ChildNode);
            }
            else if (node is MyNodeMultiCH)
            {
                MyNodeMultiCH nodeMCH = node as MyNodeMultiCH;
                if (nodeMCH.MaxTotalCHIndoorPipeLength > 0)
                {
                    double childPipeLength = nodeMCH.ActualTotalCHIndoorPipeLength;
                    if (childPipeLength > nodeMCH.MaxTotalCHIndoorPipeLength)
                    {
                        foreach (Node child in nodeMCH.ChildNodes)
                        {
                            childPipeLength = getTotalToIndoorLength(child);  //Multi-CHBOX - ... -INDOOR 的实际管长
                            if (childPipeLength > nodeMCH.MaxTotalCHIndoorPipeLength)
                            {
                                DrawNodeErrorLinks(child, curSystemItem);
                                TempActualLength = childPipeLength;
                                TempMaxLength = nodeMCH.MaxTotalCHIndoorPipeLength;
                                return PipingErrors.PIPING_CHTOINDOORTOTALLENGTH; //CH-Box到每台室内机的总管长超出限制
                            }
                        }
                    }
        ...
        /// 获取某节点到每个Indoor的管长总和，包含节点进管的长度--递归 add on 20160511 by Yunxiao Lin
        /// <summary>
        /// 获取某节点到每个Indoor的管长总和，包含节点进管的长度--递归
        /// </summary>
        public double getTotalToIndoorLength(Node node)
        {
            ...
            if (node is MyNode)
            {
                ...
                MyLink link = myNode.MyInLinks[0];
                if (link != null)
                {
                    total_length += link.Length;
                }
        ...
        public void CalcPipingLimitValues(SystemVRF sysItem, AddFlow addFlowItem)
        {
            ...
            sysItem.TotalPipeLength = 0;
            if (!sysItem.IsInputLengthManually)
            {
                //同一个Project下面可能有多个ProductType，所以需要用室外机的属性来判断 20160825 by Yunxiao lin
                foreach (Node nd in addFlowItem.Nodes)
                {
                    //如果排管存在错误，不恢复正常颜色。 modify on 20160727 by Yunxiao lin
                    if (nd is MyNodeOut || (nd.InLinks.Count > 0 &&
                        (nd.Links[0].DrawColor != utilPiping.colorWarning || sysItem.IsPipingOK)))
                    {
                        utilPiping.setItemDefault(nd);
                    }
                }
        ...
        public void SetPipingLimitation(SystemVRF sysItem)
        {
            ...
            {
                //如果MaxTotalPipeLength_MaxIU为空 则取MaxTotalPipeLength
                //如果MaxMKIndoorPipeLength_MaxIU为空 则取MaxMKIndoorPipeLength  on 20181030 by xyj
                sysItem.MaxIndoorLength = outItem.MaxIndoorLength_MaxIU == 0 ? outItem.MaxIndoorLength : outItem.MaxIndoorLength_MaxIU;
                sysItem.MaxTotalPipeLength = outItem.MaxTotalPipeLength_MaxIU == 0 ? outItem.MaxTotalPipeLength : outItem.MaxTotalPipeLength_MaxIU;
                sysItem.MaxMKIndoorPipeLength = outItem.MaxMKIndoorPipeLength_MaxIU == 0 ? outItem.MaxMKIndoorPipeLength : outItem.MaxMKIndoorPipeLength_MaxIU;
        ...
        /// 计算及检验MainBranch
        /// </summary>
        public PipingErrors ValMainBranch(SystemVRF sysItem, AddFlow addFlowPiping)
        {
                ...
                if (node is MyNodeYP)
                {
                    ...
                    if (!yp.IsCP)
                    {
                        ...
                        if (isMainbranch)
                        {
                            ...
                            //        }
                            //        else if (n is MyNodeYP && n.InLinks.Count > 0 && n.InLinks[0].Org is MyNodeOut)
                            //        {
                            //            fNode = n; //第一分歧管节点
                            //            double tempRealLength = 0;
                            ...
                            else
                            {
                                ...
                                //计算Indoor Capacity Rate，每个分支的容量比例不能小于40%
        ...
        public static void setCoolingOnlyItemColor(AddFlow addFlowPiping, Color color)
        {
            foreach (Node node in addFlowPiping.Nodes)
            {
                if (node is MyNodeIn)
        ...
        /// <param name="addFlowPiping"></param>
        /// <returns></returns>
        public bool ValCoolingOnlyIndoorCapacityRate(SystemVRF sysItem, AddFlow addFlowPiping)
        {
        ...
        /// <summary>
        /// 计算并验证CH-Box后连接的Indoor数量，本函数需要用到CH-Box允许最大连接数，所以必须放到配管计算之后执行
        /// </summary>
        /// <param name="sysItem"></param>
        /// <param name="addFlowPiping"></param>
        /// <returns></returns>
        public bool ValMaxIndoorNumberConnectToCH(SystemVRF sysItem, AddFlow addFlowPiping)
        {
            foreach (Node node in addFlowPiping.Nodes)
            {
        ...
        public PipingErrors ValMaxEqLength(SystemVRF sysItem, AddFlow addFlowPiping)
        {
            ...
            foreach (Node node in addFlowPiping.Nodes)
            {
                if (node is MyNodeIn)
                {
                    ...
                    getExtraLength(node, ref tempExtraLength);
                    getRealLength(node, ref tempRealLength);
        ...
        /// <param name="extraLength">（返回）节点到室内机之间弯头与油弯总长</param>
        private void getExtraLength(Node node, ref double extraLength)
        {
            if (node == null) return;
            if (node is MyNodeYP)
            {
            ...
            double elbowL = link.ElbowQty == 0 ? 0 : GetElbowLength(Math.Round(link.ElbowQty), gasPipeSize);
            double oilL = link.OilTrapQty == 0 ? 0 : GetOilLength(Math.Round(link.OilTrapQty), gasPipeSize);
            extraLength += elbowL + oilL;
            if (link.Org is MyNodeOut)
                return;
            else
                getExtraLength(link.Org, ref extraLength);
        }
        ...
        /// </summary>
        /// <param name="node">起始节点</param>
        /// <param name="realLength">（返回）节点到室外机的实际管长</param>
        private void getRealLength(Node node, ref double realLength)
        {
            ...
            if (link.Org is MyNodeOut)
                ...
                getRealLength(link.Org, ref realLength);
        }

        /// 将指定节点到室外机的路径标记为红色--递归
        /// 由于多ProductType功能，同一个Project下面可能存在不同的ProductType，所以需要增加productType参数 20160825 by Yunxiao Lin
        /// <summary>
        /// 将指定节点到室外机的路径标记为红色--递归
        /// </summary>
        /// <param name="node"></param>
        /// <param name="color"></param>
        private void setLinkPathColor(Node node, Color color, string productType)
        {
            foreach (Link lk in node.InLinks)
            {
        ...
        /// 将指定节点下端的所有路径标记颜色--递归
        /// </summary>
        public void setDownwardLinkPathColor(Node node, Color color)
        {
            if (node == null) return;
        ...
        /// </summary>
        public void setUpwardToNodeLinPathColor(Node nodefrom, Node nodeto, Color color)
        {
            if (nodefrom == nodeto) return;
            if (nodefrom.InLinks[0] != null)
            {
                nodefrom.InLinks[0].DrawColor = color;
                if (nodefrom.InLinks[0].Org != null)
                {
                    setUpwardToNodeLinPathColor(nodefrom.InLinks[0].Org, nodeto, color);
                }
            }
        }
        ...
        /// </summary>
        /// <returns></returns>
        public double GetPipeLengthFactor(SystemVRF sysItem, Outdoor item, string condition)
        {
            if (item == null)
                return 0;
        ...
        public void GatherPipeSpec(AddFlow addflowItem, ref Dictionary<string, double> dicSpecL, SystemVRF sysItem)
        {
            ...
            if (sysItem == null || sysItem.OutdoorItem == null)
                return;
            string fCode = GetFactoryCode(sysItem.OutdoorItem.ModelFull);
        ...
        private double AddRefrigeration(SystemVRF sysItem, string FactoryCode, double L1, double L2, double L3, double L4, double L5, double L6, double L7, double L8, double L_FSNSH3M, AddFlow addflowItem)
        {
            ...
            if ("Q".Equals(FactoryCode) && !series.Contains("IVX"))
            {
                ...
                else
                {
                    ...
                    #endregion
                    #region//FSCNY1Q Low Ambient add on 20160626 by Yunxiao Lin
                    ...
                    #endregion
                    #region//HNBRKQ,HNBRMQ Low Ambient add on 20180202 by Vince 
                    ...
                    if (mat.Success)
                    {
                        ...
                        else if (ODUModelNo >= 70 && ODUModelNo <= 120)
                        {
                            ...
                            int N3 = 0;   //80~160
                            //统计各个范围内的环绕出风式卡式机数量
                            foreach (RoomIndoor rinItem in thisProject.RoomIndoorList)
                            {
        ...
        /// add by Shen Junjie on 2018/09/26
        /// <summary>
        /// 是否某系统的室内机是否满足W1 Min amount的条件
        /// </summary>
        /// <param name="sysItem">指定的系统</param>
        /// <param name="modelsList">型号范围</param>
        /// <param name="capacityList">容量范围</param>
        /// <returns></returns>
        ...
        /// RAS-JNBBTQ和RAS-CNBCMQ的规则相同 add on 2018/09/26 by Shen Junjie
        /// </summary>
        /// <returns></returns>
        ...
        ///// <param name="g"></param>
        //public void DrawEMF_Piping(Graphics g, AddFlow addFlowPiping, SystemVRF curSystemItem, string PipingImageDir, Project thisProject, bool isInch)
        //{
        ...
        /// 绘制配管图中的 Node 节点以及 Links
        /// </summary>
        public void DrawPipingNodes(SystemVRF sysItem, string dir)
        {
            // 1、绘制室外机节点
            if (sysItem.OutdoorItem != null)
            {
                ...
                MyNodeOut nodeOut = sysItem.MyPipingNodeOut;
                addFlowPiping.Nodes.Add(nodeOut);
                drawNodeImage(nodeOut, imgFile, ref addFlowPiping);
                nodeOut.Location = new PointF(5f, 5f + UtilPiping.HeightForNodeText);
                ...
                //绘制所有节点（除室外机）的图片
                DrawChildNodesImage(nodeOut, firstChildNode, dir);

                if (sysItem.PipingLayoutType == PipingLayoutTypes.BinaryTree)
                {
                    //计算所有节点（除室外机）的位置
                    float maxY = nodeOut.Location.Y;
                    float maxX = 0;
                    LayoutBinaryTree(sysItem, nodeOut, firstChildNode, ref maxX, ref maxY);
                }
                else if (sysItem.PipingLayoutType == PipingLayoutTypes.Symmetries)
                {
                    float offset = 0;
                    LayoutSymmetries(sysItem, nodeOut, firstChildNode, false, 0, ref offset);
                }
                else if (sysItem.PipingLayoutType == PipingLayoutTypes.SchemaA)
                {
                    float minX = 0, minY = 0, offset = 0;
                    float[] maxPositions = new float[2];
                    LayoutSchemaA(sysItem, nodeOut, firstChildNode, PipingOrientation.Unknown,
                        minX, minY, ref maxPositions, ref offset);
                }
                else
                {
                    float maxY = nodeOut.Location.Y;
                    LayoutNormal(sysItem, nodeOut, firstChildNode, dir, ref maxY);
                }
                drawTextToODUNode(nodeOut, outNodeItem, sysItem);
            }
        }
        ...
        private void DrawChildNodesImage(Node parent, Node node, string dir)
        {
            if (node == null) return;
            if (node is MyNodeYP)
            {
                ...
                for (int i = 0; i < nodeYP.MaxCount; ++i)
                {
                    DrawChildNodesImage(nodeYP, nodeYP.ChildNodes[i], dir);
                }
            }
            else if (node is MyNodeIn)
            {
            ...
            else if (node is MyNodeCH)
            {
                ...
                DrawChildNodesImage(nodeCH, nodeCH.ChildNode, dir);
            }
            else if (node is MyNodeMultiCH)
            {
                MyNodeMultiCH nodeMCH = node as MyNodeMultiCH;
                ...
                for (int i = 0; i < nodeMCH.ChildNodes.Count; ++i)
                {
                    DrawChildNodesImage(nodeMCH, nodeMCH.ChildNodes[i], dir);
                }
            }
        }
        ...
        /// </summary>
        private void LayoutBinaryTree(SystemVRF sysItem, Node parent, Node node, ref float maxX, ref float maxY)
        {
            if (node == null) return;
            ...
            if (node is MyNodeYP)
            {
                ...
                for (int i = 0; i < nodeYP.MaxCount; ++i)
                {
                    LayoutBinaryTree(sysItem, nodeYP, nodeYP.ChildNodes[i], ref maxX, ref maxY);
                }
            ...
            {
                MyNodeCH nodeCH = node as MyNodeCH;
                LayoutBinaryTree(sysItem, nodeCH, nodeCH.ChildNode, ref maxX, ref maxY);
            ...
            else if (node is MyNodeMultiCH)
            {
                ...
                for (int i = 0; i < nodeMCH.ChildNodes.Count; ++i)
                {
                    LayoutBinaryTree(sysItem, nodeMCH, nodeMCH.ChildNodes[i], ref maxX, ref maxY);
                }

                //子节点排列好以后，再计算Multi CH Box节点的坐标 
                PointF ptFirstChild = utilPiping.getCenterPointF(nodeMCH.ChildNodes[0]);
                PointF ptLastChild = utilPiping.getCenterPointF(nodeMCH.ChildNodes[nodeMCH.ChildNodes.Count - 1]);

                if (isVertical)
                {
                    nodeMCH.Location = new PointF(nodeMCH.Location.X, (ptFirstChild.Y + ptLastChild.Y - nodeMCH.Size.Height) / 2);
                }
                else
                {
                    nodeMCH.Location = new PointF((ptFirstChild.X + ptLastChild.X - nodeMCH.Size.Width) / 2, nodeMCH.Location.Y);
                }
            }
        }
        ...
        /// </summary>
        private void LayoutSymmetries(SystemVRF sysItem, Node parent, Node node, bool forward, float minXY, ref float offset)
        {
            if (node == null) return;
            bool isVertical = sysItem.IsPipingVertical;
            float subOffset = 0;
            if (parent is MyNodeOut)
            {
                ...
                {
                    //最小Y坐标
                    minXY = parent.Location.Y + parent.Size.Height + 40;
                }
            }
            ...
            if (node is MyNodeYP)
            {
                ...
                if (ypNode.IsCP)
                {
                    for (int i = 0; i < ypNode.MaxCount; ++i)
                    {
                        bool tempForward = (i % 2 == 1) ? !forward : forward;
                        LayoutSymmetries(sysItem, ypNode, ypNode.ChildNodes[i], tempForward, minXY, ref subOffset);
                    }
                }
                else
                {
                    LayoutSymmetries(sysItem, ypNode, ypNode.ChildNodes[0], forward, minXY, ref subOffset);

                    if (!forward && subOffset != 0)
                    {
                        if (isVertical)
                        {
                            node.Location = new PointF(node.Location.X + subOffset, node.Location.Y);
                        }
                        else
                        {
                            node.Location = new PointF(node.Location.X, node.Location.Y + subOffset);
                        }
                        offset += subOffset;
                    }
                    subOffset = 0;
                    LayoutSymmetries(sysItem, ypNode, ypNode.ChildNodes[1], !forward, minXY, ref subOffset);
                }
            }
            else if (node is MyNodeCH)
            {
                MyNodeCH nodeCH = node as MyNodeCH;
                LayoutSymmetries(sysItem, nodeCH, nodeCH.ChildNode, forward, minXY, ref subOffset);
        ...
        /// 计算所有节点（除室外机）的位置 (对称布局)
        /// </summary>
        private void LayoutSchemaA(SystemVRF sysItem, Node parent, Node node, PipingOrientation orientation,
            float minX, float minY, ref float[] maxPositions, ref float offset)
        {
            if (node == null) return;
            bool isVertical = sysItem.IsPipingVertical;
            float subOffset = 0;
            if (parent is MyNodeOut)
            {
                if (isVertical)
                {
                    ...
                    {
                        parent.Location = new PointF(200f, 5f + UtilPiping.HeightForNodeText);
                    }
                    orientation = PipingOrientation.Left;
                }
                else
                {
                    //最小Y坐标
                    minY = parent.Location.Y + parent.Size.Height + 40;
                    orientation = PipingOrientation.Down;
                }
            }

            addFlowPiping.Nodes.Add(node);

            if (orientation == PipingOrientation.Right || orientation == PipingOrientation.Left)
            {
                //纵向布局
                bool orientateRight = orientation == PipingOrientation.Right;
            ...
            {
                //横向布局
                bool orientateUp = orientation == PipingOrientation.Up;
            ...
            if (node is MyNodeYP)
            {
                ...
                if (nodeYP.IsCP)
                {
                    for (int i = 0; i < nodeYP.MaxCount; ++i)
                    {
                        LayoutSchemaA(sysItem, nodeYP, nodeYP.ChildNodes[i], orientation, minX, minY, ref maxPositions, ref subOffset);
                    }
                }
                else
                {
                    PipingOrientation orientation1 = orientation;
                    PipingOrientation orientation2 = orientation;
                    Node child1, child2;
                    ...
                    if (isVertical)
                    {
                        if (parent is MyNodeOut)
                        {
                            ...
                            orientation2 = PipingOrientation.Right;
                        }
                    }
                    else
                    {
                        if (parent is MyNodeOut)
                        {
                            orientation1 = PipingOrientation.Up;
                        }
                    }
                    LayoutSchemaA(sysItem, nodeYP, child1, orientation1, minX, minY, ref maxPositions, ref subOffset);
                    LayoutSchemaA(sysItem, nodeYP, child2, orientation2, minX, minY, ref maxPositions, ref subOffset);
                }
            }
            else if (node is MyNodeCH)
            {
                ...
                if (parent is MyNodeOut)
                {
                    orientation1 = isVertical ? PipingOrientation.Right : PipingOrientation.Down;
                }
                LayoutSchemaA(sysItem, nodeCH, nodeCH.ChildNode, orientation1, minX, minY, ref maxPositions, ref subOffset);
            }
            else if (node is MyNodeMultiCH)
            {
                MyNodeMultiCH nodeMCH = node as MyNodeMultiCH;
                ...
                for (int i = 0; i < nodeMCH.ChildNodes.Count; ++i)
                {
                    LayoutSchemaA(sysItem, nodeMCH, nodeMCH.ChildNodes[i], orientation1, minX, minY, ref maxPositions, ref subOffset);
                }
            }
            ...
            if (subOffset > 0)
            {
                if (isVertical)
                {
                    parent.Location = new PointF(parent.Location.X + subOffset, parent.Location.Y);
                }
                else if (!(parent is MyNodeOut))
                {
                    parent.Location = new PointF(parent.Location.X, parent.Location.Y + subOffset);
                }
                offset += subOffset;
            }
        }
        ...
        /// </summary>
        /// <param name="pn"></param>
        private void LayoutNormal(SystemVRF sysItem, Node parent, Node node, string dir, ref float maxY)
        {
            if (node == null) return;
            addFlowPiping.Nodes.Add(node);
            ...
            if (node is MyNodeYP)
            {
                ...
                if (isVertical)
                {
                    for (int i = 0; i < nodeYP.MaxCount; ++i)
                    {
                        LayoutNormal(sysItem, nodeYP, nodeYP.ChildNodes[i], dir, ref maxY);
                        ...
                        if (i == 0)
                        {
                            ...
                            {
                                node.Location = new PointF(node.Location.X, ptFirstChild.Y - node.Size.Height / 2);
                            }
                        }
                    }
                }
            ...
            else if (node is MyNodeCH)
            {
                ...
                LayoutNormal(sysItem, nodeCH, nodeCH.ChildNode, dir, ref maxY);
            }
            else if (node is MyNodeMultiCH)
            {
                MyNodeMultiCH nodeMCH = node as MyNodeMultiCH;
                ...
                for (int i = 0; i < nodeMCH.ChildNodes.Count; i++)
                {
                    LayoutNormal(sysItem, nodeMCH, nodeMCH.ChildNodes[i], dir, ref maxY);
                }

                if (node is MyNodeMultiCH)
                {
                    maxY = Math.Max(maxY, node.Location.Y + node.Size.Height - 40);
                }
            }
            else if (node is MyNodeIn)
            {
        ...
        /// </summary>
        /// <param name="node"></param>
        /// <param name="imgFile">带路径文件名</param>
        /// <param name="addFlowPiping"></param>
        public void drawNodeImage(Node node, string imgFile, ref AddFlow addFlowPiping)
        {
        ...
        /// 绘制配管图中的节点之间的连接线
        /// </summary>
        public void DrawPipingLinks(SystemVRF sysItem, AddFlow addFlowPiping)
        {
            MyNodeOut nodeOut = sysItem.MyPipingNodeOut;
            ...
            {
                //二叉树布局节点连线
                DrawLinkForBinaryTree(nodeOut, firstNode, sysItem);
            }
            else if (sysItem.PipingLayoutType == PipingLayoutTypes.Symmetries)
            {
                DrawLinkForSymmetries(nodeOut, firstNode, 0, sysItem, false);
            }
            else if (sysItem.PipingLayoutType == PipingLayoutTypes.SchemaA)
            {
                PipingOrientation orientation = sysItem.IsPipingVertical ? PipingOrientation.Left : PipingOrientation.Down;
                DrawLinkForSchemaA(nodeOut, firstNode, 0, sysItem, orientation);
            }
            else
            {
                //普通布局节点连线
                DrawLinkForNormal(nodeOut, firstNode, true, sysItem);
            }
        }
        ...
        /// 绘制节点之间的连接线
        /// </summary>
        private void DrawLinkForBinaryTree(Node parent, Node node, SystemVRF sysItem)
        {
            if (node == null) return;
                ...
                for (int linkIndex = 0; linkIndex < myNode.MyInLinks.Count; ++linkIndex)
                {
                    ...
                    if (node is MyNodeIn || node is MyNodeYP)
                    {
                        ...
                        else if (parent is MyNodeMultiCH)
                        {
                            ...
                            float pheight = parent.Size.Height / (parentMCH.ChildNodes.Count + 1);
                            ...
                            if (isVertical)
                            {
                                ...
                                {
                                    myLink.Points[1] = new PointF(myLink.Points[0].X + 10 + 3 * (parentMCH.ChildNodes.Count - nodeIndex), myLink.Points[0].Y);
                                }
                                else
                                {
                                    myLink.Points[1] = new PointF(myLink.Points[0].X + 10 + 3 * (nodeIndex + 1), myLink.Points[0].Y);
                                }
                                myLink.Points[2] = new PointF(myLink.Points[1].X, myLink.Points[3].Y);
                            }
                            else
                            {
                                PointF ptNode = utilPiping.getCenterPointF(node);
                                ...
                                else
                                {
                                    myLink.Points[0] = utilPiping.getBottomCenterPointF(parent);
                                    myLink.Line.Style = LineStyle.VHV;
                                    myLink.Points[1] = new PointF(myLink.Points[0].X, myLink.Points[0].Y + 3);
                                }
                            }
                        }
                        else if (parent is MyNodeYP)
                        {
                            ...
                            if (linkIndex == 1)
                            {
                                if (isVertical)
                                {
                                    myLink.Points[0] = new PointF(myLink.Points[0].X + 3, myLink.Points[0].Y - 3);
                                    myLink.Points[1] = new PointF(myLink.Points[1].X + 3, myLink.Points[1].Y - 3);
                                    myLink.Points[2] = new PointF(myLink.Points[2].X, myLink.Points[2].Y - 3);
                                }
                                else
                                {
                                    myLink.Points[0] = new PointF(myLink.Points[0].X + 3, myLink.Points[0].Y);
                                    myLink.Points[1] = new PointF(myLink.Points[1].X + 3, myLink.Points[1].Y);
                                    myLink.Points[2] = new PointF(myLink.Points[2].X + 3, myLink.Points[2].Y);
                                }
                            }
                        }
                    }
                    else if (node is MyNodeCH)
                    {
                        ...
                        if (parent is MyNodeOut)
                        {
                            ...
                            if (isVertical)
                            {
                                //修复纵向布局时Outdoor连接CHBox的连线错误 add by Shen Junjie on 20171022
                                myLink.Points[myLink.Points.Count - 1] = utilPiping.getLeftCenterPointF(node);
                                myLink.Line.Style = LineStyle.VH;
                            }
                            else
            ...
            if (node is MyNodeYP)
            {
                ...
                for (int i = 0; i < nodeYP.MaxCount; ++i)
                {
                    DrawLinkForBinaryTree(nodeYP, nodeYP.ChildNodes[i], sysItem);
                }
            }
            else if (node is MyNodeCH)
            {
                MyNodeCH chboxNode = node as MyNodeCH;
                DrawLinkForBinaryTree(chboxNode, chboxNode.ChildNode, sysItem);
            }
            else if (node is MyNodeMultiCH)
            {
                MyNodeMultiCH nodeMCH = node as MyNodeMultiCH;
                for (int i = 0; i < nodeMCH.ChildNodes.Count; ++i)
                {
                    DrawLinkForBinaryTree(nodeMCH, nodeMCH.ChildNodes[i], sysItem);
                }
            }
        }
        ...
        /// 绘制节点之间的连接线
        /// </summary>
        private void DrawLinkForSymmetries(Node parent, Node node, int nodeIndex, SystemVRF sysItem, bool forward)
        {
            if (node == null) return;
                    ...
                    else if (node is MyNodeIn)
                    {
                        ...
                        else if (parent is MyNodeYP)
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    myLink.Line.Style = LineStyle.HVH; // 该行代码必须在重置连接线顶点的坐标之后执行
                                }
                            }
                        }
                        else if (parent is MyNodeCH)
            ...
            if (node is MyNodeYP)
            {
                ...
                {
                    bool tempForward = (i % 2 == 1) ? !forward : forward;
                    DrawLinkForSymmetries(nodeYP, nodeYP.ChildNodes[i], i, sysItem, tempForward);
                }
            }
            else if (node is MyNodeCH)
            {
                MyNodeCH nodeCH = node as MyNodeCH;
                DrawLinkForSymmetries(nodeCH, nodeCH.ChildNode, 0, sysItem, forward);
            }
        }
        ...
        /// 绘制节点之间的连接线
        /// </summary>
        private void DrawLinkForSchemaA(Node parent, Node node, int nodeIndex, SystemVRF sysItem, PipingOrientation orientation)
        {
            if (node == null) return;
            ...
            if (node is MyNode)
            {
                ...
                for (int linkIndex = 0; linkIndex < myNode.MyInLinks.Count; ++linkIndex)
                {
                    ...
                    if (node is MyNodeYP)
                    {
                        ...
                        {
                            if (orientation == PipingOrientation.Left)
                            {
                                myLink.Points[0] = utilPiping.getLeftCenterPointF(parent);
                            }
                            else if (orientation == PipingOrientation.Right)
                            {
                                myLink.Points[0] = utilPiping.getRightCenterPointF(parent);
                            }
                            else if (orientation == PipingOrientation.Up)
                            {
                                myLink.Points[0] = utilPiping.getTopCenterPointF(parent);
                            }
                            else if (orientation == PipingOrientation.Down)
                            {
                                myLink.Points[0] = utilPiping.getBottomCenterPointF(parent);
                            }
                        }
                        else if (parent is MyNodeMultiCH)
                        {
                            ...
                            {
                                if (orientation == PipingOrientation.Up)
                                {
                                    pt1.Y = parent.Location.Y + parent.Size.Height / (parentMCH.ChildNodes.Count + 1) * (nodeIndex + 1);
                                }
                                else
                            ...
                            if (isVertical)
                            {
                                if (orientation == PipingOrientation.Left)
                                {
                                    myLink.Points[1] = new PointF(myLink.Points[0].X - 3 * (parentMCH.ChildNodes.Count - nodeIndex), myLink.Points[0].Y);
                                }
                                else
                                {
                                    myLink.Points[1] = new PointF(myLink.Points[0].X + 3 * (parentMCH.ChildNodes.Count - nodeIndex), myLink.Points[0].Y);
                            ...
                            {
                                myLink.Points[1] = new PointF(myLink.Points[3].X - 30, myLink.Points[0].Y);
                                myLink.Points[2] = new PointF(myLink.Points[1].X, myLink.Points[3].Y);
                            }
                        }
                        #endregion
                    ...
                    else if (node is MyNodeIn)
                    {
                        ...
                        else if (parent is MyNodeYP)
                        {
                            ...
                            if (isVertical)
                            {
                                if (orientation == PipingOrientation.Right)
                                {
                                    myLink.Points[myLink.Points.Count - 1] = utilPiping.getLeftCenterPointF(node);
                                }
                                else
                            ...
                            if (nodeIndex == 0 && linkIndex == 1)
                            {
                                ...
                                {
                                    myLink.Points[0] = new PointF(myLink.Points[0].X + 3, myLink.Points[0].Y - 3);
                                    myLink.Points[1] = new PointF(myLink.Points[1].X, myLink.Points[1].Y - 3);
                                }
                                else
                                {
                                    myLink.Points[0] = new PointF(myLink.Points[0].X + 3, myLink.Points[0].Y);
                                    if (orientation == PipingOrientation.Down)
                                    {
                                        myLink.Points[1] = new PointF(myLink.Points[1].X, myLink.Points[1].Y - 3);
                                    }
                                    else if (orientation == PipingOrientation.Up)
                            ...
                            else
                            {
                                ...
                                {
                                    PointF endPoint = utilPiping.getCenterPointF(node);
                                    myLink.Line.Style = LineStyle.HVH; // 该行代码必须在重置连接线顶点的坐标之后执行
                                    myLink.Points[1] = new PointF(endPoint.X - 50, myLink.Points[0].Y);
                                    myLink.Points[2] = new PointF(endPoint.X - 50, myLink.Points[myLink.Points.Count - 1].Y);
                                }
                            }
                        }
                        else if (parent is MyNodeCH)
                        {
                            if (isVertical)
                            {
                                ...
                                else if (orientation == PipingOrientation.Left)
                                {
                                    myLink.Points[0] = utilPiping.getLeftCenterPointF(parent);
                                    myLink.Points[myLink.Points.Count - 1] = utilPiping.getRightCenterPointF(node);
                                }
            ...
            if (node is MyNodeYP)
            {
                ...
                for (int i = 0; i < nodeYP.MaxCount; ++i)
                {
                    ...
                    if (isFirstYP && i == 0)
                    {
                        orientation1 = isVertical ? PipingOrientation.Right : PipingOrientation.Up;
                    }
                    DrawLinkForSchemaA(nodeYP, nodeYP.ChildNodes[i], i, sysItem, orientation1);
                }
            }
            else if (node is MyNodeCH)
            {
                ...
                DrawLinkForSchemaA(nodeCH, nodeCH.ChildNode, 0, sysItem, orientation1);
            }
            else if (node is MyNodeMultiCH)
            {
                MyNodeMultiCH nodeMCH = node as MyNodeMultiCH;
                ...
                for (int i = 0; i < nodeMCH.ChildNodes.Count; ++i)
                {
                    DrawLinkForSchemaA(nodeMCH, nodeMCH.ChildNodes[i], i, sysItem, orientation1);
                }
            }
        }
        ...
        /// 递归程序，绘制YP节点组的连接线
        /// </summary>
        private void DrawLinkForNormal(Node parent, Node node, bool isFirstChild, SystemVRF sysItem)
        {
            if (node == null) return;
            ...
            if (node is MyNode)
            {
                ...
                for (int linkIndex = 0; linkIndex < myNode.MyInLinks.Count; ++linkIndex)
                {
                    ...
                    else if (parent is MyNodeYP)
                    {
                        ...
                        PointF ptf = utilPiping.getCenterPointF(parent);

                        // 竖直布局处理
                        isFirstChild = yp.GetIndex(node) == 0;
                        if (isVertical && isFirstChild && yp.IsCP)
                        {
                            ptf.Y = parent.Location.Y + 10;
                        }
                        myLink.Points[0] = ptf;
                    }
                    else if (parent is MyNodeCH)
                    ...
                    {
                        MyNodeMultiCH parentMCH = parent as MyNodeMultiCH;
                        PointF pt1 = utilPiping.getRightTopPointF(parent);
                        int nodeIndex = parentMCH.GetIndex(node);
                    ...
                    else if (node is MyNodeMultiCH)
                    {
                        ...
                        else
                        {
                            ...
                            if (parent is MyNodeOut)
                            {
                                myLink.Line.Style = LineStyle.VHV;
                                myLink.Points[1] = new PointF(myLink.Points[0].X, myLink.Points[0].Y + 10);
                                myLink.Points[2] = new PointF(myLink.Points[3].X, myLink.Points[1].Y);
                            }
                            else
                            {
                                myLink.Line.Style = LineStyle.HV;
                    ...
                    else if (node is MyNodeIn)
                    {
                        ...
                        {
                            if (!isVertical && !isFirstChild)    // 横向排列时的第二个室内机节点的连线，折点位置调整
                            {
                                myLink.Line.Style = LineStyle.HVH;
                                int index = 0;
                                MyNodeYP parentYP = parent as MyNodeYP;
            ...
            if (node is MyNodeYP)
            {
                ...
                for (int i = 0; i < nodeYP.MaxCount; ++i)
                {
                    DrawLinkForNormal(nodeYP, nodeYP.ChildNodes[i], (i == 0), sysItem);
                }
            }
            else if (node is MyNodeCH)
            {
                MyNodeCH nodeCH = node as MyNodeCH;
                DrawLinkForNormal(nodeCH, nodeCH.ChildNode, isFirstChild, sysItem);
            }
            else if (node is MyNodeMultiCH)
            {
                MyNodeMultiCH nodeMCH = node as MyNodeMultiCH;
                for (int i = 0; i < nodeMCH.ChildNodes.Count; ++i)
                {
                    DrawLinkForNormal(nodeMCH, nodeMCH.ChildNodes[i], (i == 0), sysItem);
                }
            }
        }
        #endregion
        ...
        public void drawTextToIDUNode(SystemVRF sysItem, MyNodeIn nodeUnit)
        {
            ...
            string text = "";
            RoomIndoor riItem = nodeUnit.RoomIndooItem;
            ...
            if (label.Size.Width < nodeUnit.Size.Width)
                label.Size = new SizeF(nodeUnit.Size.Width, label.Size.Height);
            ...
            label2.Alignment = Alignment.LeftJustifyMIDDLE;
            if (label2.Size.Width < nodeUnit.Size.Width)
            {
                label2.Size = new SizeF(nodeUnit.Size.Width, label2.Size.Height);
            }

            addFlowPiping.Nodes.Add(label2);
            label2.Parent = nodeUnit;

            label.Location = new PointF(nodeUnit.Location.X, nodeUnit.Location.Y - label.Size.Height);
            label2.Location = new PointF(nodeUnit.Location.X, nodeUnit.Location.Y + nodeUnit.Size.Height);
            //Math.Max(nodeUnit.Location.X - 10, nodeUnit.Location.X + (nodeUnit.Size.Width - label2.Size.Width) / 2),

            if (sysItem.PipingLayoutType == PipingLayoutTypes.Normal)
            {
                if (!sysItem.IsPipingVertical)
                {
                    label2.Location = new PointF(label2.Location.X, label2.Location.Y + 8);
                }
            }
            else if (sysItem.PipingLayoutType == PipingLayoutTypes.BinaryTree)
            {
                if (!sysItem.IsPipingVertical)
                {
                    label.Location = new PointF(nodeUnit.Location.X, nodeUnit.Location.Y + nodeUnit.Size.Height);
                    label2.Location = new PointF(label2.Location.X, label2.Location.Y + label.Size.Height);
                }
            }
        }
        ...
        /// <param name="nodeUnit"></param>
        /// <param name="font"></param>
        public void drawTextToODUNode(MyNodeOut nodeUnit, NodeElement_Piping outNodeItem, SystemVRF sysItem)
        {
            if (nodeUnit.Children != null)
            {
                while (nodeUnit.Children.Count > 0)
                {
            ...
            addFlowPiping.Nodes.Add(label);
            label.Parent = nodeUnit;
            float yModel = nodeUnit.Location.Y - UtilPiping.HeightForNodeText;
            if (outNodeItem.PtModelLocation != null && outNodeItem.PtModelLocation.Count == 4)
            {
                yModel = 1;
            }
            label.Location = new PointF(nodeUnit.Location.X, yModel);//nodeUnit.Location.Y - UtilPiping.HeightForNodeText
            nodeUnit.HighlightChildren = true;
            ...
            foreach (string modelName in outModelArry)
            {
                ...
                else
                {
                    ...
                    if (lbx == nodeUnit.Location.X)
                    {
                        ...
                        {
                            if (outModelArry.Length > 3)
                                lbx = nodeUnit.Location.X + 110;
                            else
                                lbx = nodeUnit.Location.X + 70;
                        }
                        else
                            lbx = nodeUnit.Location.X + 55;
                    }
                }
                ...
                addFlowPiping.Nodes.Add(label);
                label.Parent = nodeUnit;
                //label.Location = new PointF(lbx, nodeUnit.Location.Y + 20);
                label.Location = new PointF(lbx, nodeUnit.Location.Y - 12);   //piping室外机分机型号纵轴位置微调
                if (outNodeItem.Name.Contains("HAPB_WS") || outNodeItem.Name.Contains("NA")) //存在特殊多联机图微调X位置
                    lbx += 45;
                else
                    lbx += 65;
            ...
            addFlowPiping.Nodes.Add(label);
            label.Parent = nodeUnit;
            ptText = new PointF(nodeUnit.Location.X + nodeUnit.Size.Width + 10, nodeUnit.Location.Y + 20);
        ...
        void initTextNode(Node label, string text)
        {
            // 设置字体并量取文字标题的尺寸
            Graphics g = addFlowPiping.CreateGraphics();
            g.PageUnit = GraphicsUnit.Pixel;            //单位为像素
            label.Font = utilPiping.textFont_piping;
            label.TextColor = utilPiping.colorText;
            label.Transparent = true;
            label.Text = text;
            SizeF size = g.MeasureString(text, utilPiping.textFont_piping);
            label.Size = new SizeF(size.Width + 5, size.Height + 3);
        ...
        public void DrawTextToAllNodes(Node node, Node parent, SystemVRF sysItem)
        {
            if (node is MyNodeOut)
            {
                MyNodeOut nodeOut = node as MyNodeOut;
        ...
        public List<PipingOrientation> CheckYPLinkOrientation(MyNodeYP nodeYP)
        {
            List<PipingOrientation> result = new List<PipingOrientation>();
            for (int i = 0; i < nodeYP.Links.Count; i++)
            {
                PipingOrientation result1 = CheckLinkOrientation(nodeYP, nodeYP.Links[i]);
                if (result1 != PipingOrientation.Unknown)
                {
                    result.Add(result1);
                }
            }
        ...
        private void drawTextToLink(MyLink myLink, int linkIndex, Node parent, Node node, bool isInch, SystemVRF sysItem)
        {
            // 绘制连接管管径文字
            ...
            //因为可以改变室内机出现型号和管径不同步的现象，未验证通过时，Indoor 也不要显示管径了。 add by Shen Junjie on 2018/5/29
            ...
            PipeCombineType pipesType = myNode.PipesType;

            float width = 0;  //预设最大宽度值
            float height = 5; //预设最大高度值
            Node label1 = new Node(); //Low Gas Pipe Size
            Node label2 = new Node(); //High Gas Pipe Size


            switch (pipesType)
            {
                case PipeCombineType.HP_L_G:  //Heat pump 2 pipes
                case PipeCombineType.HR_L_G:  //Heat recoery 2 pipes
                case PipeCombineType.HR_L_LG:  //Heat recoery cooling only
                    DrawPipeLableColor(label1, string.IsNullOrEmpty(p1) ? p2 : p1, node, 4);
                    height += 8;
                    break;
                case PipeCombineType.HR_L_LG_HG:  //Heat recoery 3 pipes
                    if (!string.IsNullOrEmpty(p1))
                    {
                        DrawPipeLableColor(label1, p1, node, 1);
                        height += 8;
                    }
                    if (!string.IsNullOrEmpty(p2))
                    {
                        DrawPipeLableColor(label2, p2, node, 2);
                        height += 8;
                    }
                    break;
            }
            ...
            //        DrawPipeLableColor(label1, p1, node, 4);
            //    else
            //        DrawPipeLableColor(label1, p1, node, 1);
            //    height += 8;
            //}
            ...
            //        DrawPipeLableColor(label2, p2, node, 4);
            ...
            Node label3 = new Node(); //Liquid Pipe Size
            if (!string.IsNullOrEmpty(p3))
            {
                DrawPipeLableColor(label3, p3, node, 3);
                height += 8;
            }

            Node label5 = new Node(); //Pipe Length
            if (!string.IsNullOrEmpty(p5))
            {
                DrawPipeLableColor(label5, p5, node, 5);
                height += 8;
            }
            width = Math.Max(Math.Max(label1.Size.Width, label2.Size.Width), Math.Max(label3.Size.Width, label5.Size.Width));   //预设最大宽度值
            label1.Size = new SizeF(width, height);
        ...
        private void SetLinkTextLocationForBinaryTree(MyLink myLink, int linkIndex, Node parent, Node node, Node label1, bool isVertical)
        {
        ...
        private void SetLinkTextLocationForSechmaA(MyLink myLink, int linkIndex, Node parent, Node node, Node label1, bool isVertical)
        {
            PointF ptText, ptParent, ptNode;
            ptText = utilPiping.getLeftCenterPointF(node);
        ...
        private void SetLinkTextLocationForNormal(MyLink myLink, int linkIndex, Node parent, Node node, Node label1, bool isVertical)
        {
            PointF ptText, ptParent, ptNode;
            ptText = utilPiping.getLeftCenterPointF(node);
            if (node is MyNodeYP)
            {
                ...
                {
                    PointF pt1 = myLink.Points[myLink.Points.Count - 2];
                    PointF pt2 = myLink.Points[myLink.Points.Count - 1];
                    ptText = new PointF((pt1.X + pt2.X - label1.Size.Width) / 2, pt2.Y);
                }
                else
                {
                    ptNode = utilPiping.getCenterPointF(node);
        ...
        private void drawTextToOtherNode(Node node, Node parent, SystemVRF sysItem)
        {
            ...
            if (node is MyNodeYP)
            {
                ...
                if (!string.IsNullOrEmpty(nodeYP.Model) && sysItem.IsPipingOK)
                {
                    ...
                    label1.Location = new PointF(nodeYP.Location.X + 10, nodeYP.Location.Y - label1.Size.Height);
                    if (sysItem.PipingLayoutType == PipingLayoutTypes.BinaryTree)
                    {
                        if (sysItem.IsPipingVertical)
                        {
                            //二叉树垂直布局 YP的型号标注放到左边
                            label1.Location = new PointF(nodeYP.Location.X - label1.Size.Width + 10, label1.Location.Y);
                        }
                    }
                    else if (sysItem.PipingLayoutType == PipingLayoutTypes.SchemaA)
                    {
                        if (!sysItem.IsPipingVertical)
                        {
                            ...
                            label1.Location = new PointF(label1.Location.X + 3, label1.Location.Y);
                        }
                    }
                    addFlowPiping.Nodes.Add(label1);
                    label1.Parent = nodeYP;
                }
            }
            else if (node is MyNodeCH)
            {
                ...
                if (!string.IsNullOrEmpty(nodeCH.Model))
                {
                    if (sysItem.IsPipingOK)
                    {
                        ...
                        if (!sysItem.IsPipingVertical)
                            label1.Location = new PointF(nodeCH.Location.X + nodeCH.Size.Width / 2 + 5, nodeCH.Location.Y - label1.Size.Height);
                        else
                            label1.Location = new PointF(nodeCH.Location.X + nodeCH.Size.Width / 2 - label1.Size.Width / 2, nodeCH.Location.Y - label1.Size.Height);
                        addFlowPiping.Nodes.Add(label1);
                        label1.Parent = nodeCH;
                    }
                }
            }
            else if (node is MyNodeMultiCH)
            {
                ...
                if (!string.IsNullOrEmpty(nodeMCH.Model))
                {
                    if (sysItem.IsPipingOK)
                    {
                        ...
                        if (sysItem.IsPipingVertical)
                        {
                            label1.Location = new PointF(nodeMCH.Location.X + nodeMCH.Size.Width / 2 - label1.Size.Width / 2, nodeMCH.Location.Y - label1.Size.Height);
                        }
                        else
                        {
                            ...
                            {
                                label1.Location = new PointF(nodeMCH.Location.X + nodeMCH.Size.Width / 2 + 5, nodeMCH.Location.Y - label1.Size.Height);
                            }
                        }
                        addFlowPiping.Nodes.Add(label1);
                        label1.Parent = nodeMCH;
                    }
                }
            }
        ...
        /// 为管线图添加Correction Factor标注 2016-4-18 by lin
        /// <summary>
        /// 为管线图添加Correction Factor标注
        /// </summary>
        public void DrawCorrectionFactorText(SystemVRF sysItem)
        {
            ...
            SizeF diagramSize = addFlowPiping.GetDiagramSize();
            legendLeft = diagramSize.Width + 20;  //Correction Factor, 冷媒标注， HR图例等等的X坐标

            MyNodeLegend label1 = new MyNodeLegend();
            //由于CAD对中文的支持不好，去掉CAD文字国际化
            //initTextNode(label1, Msg.GetResourceString("PIPING_CORRECTION_FACTOR_TEXT") + ": " + curSystemItem.PipingLengthFactor.ToString("n3"));
            initTextNode(label1, "Piping Correction Factor(Cooling): " + sysItem.PipingLengthFactor.ToString("n3"));
            label1.Location = new PointF(legendLeft, 15);
            addFlowPiping.Nodes.Add(label1);
            ...
            label2.Location = new PointF(legendLeft, 30);
            addFlowPiping.Nodes.Add(label2);
        }
        ...
        /// 为管线图添加加注冷媒标注
        /// </summary>
        public void DrawAddRefrigerationText(SystemVRF sysItem)
        {
            Node label1 = null;
            ...
            if (nodeOut.Children != null)
            {
                for (int i = 0; i < nodeOut.Children.Count; i++)
                {
                    if (nodeOut.Children[i].Tooltip == title)
                    {
        ...
        /// 为管线图添加线型图例
        /// </summary>
        public void drawPipelegend(bool isHR)
        {
            if (isHR)
            {
                MyNodeLegend label1 = new MyNodeLegend();
                ...
                Link link2 = new Link();
                Link link3 = new Link();
                Node node11 = new Node(legendLeft, 65, 1, 1);
                Node node12 = new Node(legendLeft + 100, 65, 1, 1);
                Node node21 = new Node(legendLeft, 80, 1, 1);
                Node node22 = new Node(legendLeft + 100, 80, 1, 1);
                Node node31 = new Node(legendLeft, 95, 1, 1);
                Node node32 = new Node(legendLeft + 100, 95, 1, 1);

                node11.DrawColor = utilPiping.colorTransparent;
                node12.DrawColor = utilPiping.colorTransparent;
                node21.DrawColor = utilPiping.colorTransparent;
                node22.DrawColor = utilPiping.colorTransparent;
                node31.DrawColor = utilPiping.colorTransparent;
                node32.DrawColor = utilPiping.colorTransparent;
                ...
                link1.DrawColor = utilPiping.color_piping_3pipe;
                link2.DrawColor = utilPiping.color_piping_2pipe;
                link3.DrawColor = utilPiping.color_piping_2pipe_lowgas;
                link1.DrawWidth = 1;
                link2.DrawWidth = 1;
                link3.DrawWidth = 1;
                link1.DashStyle = DashStyle.Solid;
                link2.DashStyle = DashStyle.Solid;
                link3.DashStyle = DashStyle.Solid;
                ...
                DrawPipeLableColor(label6, "Low Pressure Gas pipe : Light purple", null, 1);
                DrawPipeLableColor(label7, "High Pressure Gas pipe : Blue", null, 2);
                DrawPipeLableColor(label8, "Liquid pipe : Green", null, 3);
                DrawPipeLableColor(label9, "Gas pipe : Red", null, 4);

                label1.Location = new PointF(legendLeft + 110, 60);
                label2.Location = new PointF(legendLeft + 110, 75);
                label3.Location = new PointF(legendLeft + 110, 90);
                //label4.Location = new PointF(legendLeft, 105);
                //label5.Location = new PointF(legendLeft, 125);
                label6.Location = new PointF(legendLeft, 105);
                label7.Location = new PointF(legendLeft, 120);
                label9.Location = new PointF(legendLeft, 135);
                label8.Location = new PointF(legendLeft, 150);

                addFlowPiping.Nodes.Add(label1);
                addFlowPiping.Nodes.Add(label2);
                addFlowPiping.Nodes.Add(label3);
                //addFlowPiping.Nodes.Add(label4);
                //addFlowPiping.Nodes.Add(label5);

                addFlowPiping.Nodes.Add(node11);
                addFlowPiping.Nodes.Add(node12);
                addFlowPiping.Nodes.Add(node21);
                addFlowPiping.Nodes.Add(node22);
                addFlowPiping.Nodes.Add(node31);
                addFlowPiping.Nodes.Add(node32);

                addFlowPiping.AddLink(link1, node11, node12);
                addFlowPiping.AddLink(link2, node21, node22);
                addFlowPiping.AddLink(link3, node31, node32);
            }
            else
            {
                MyNodeLegend label8 = new MyNodeLegend();   //液管
                ...
                DrawPipeLableColor(label8, "Liquid pipe : Green", null, 3);
                DrawPipeLableColor(label9, "Gas pipe : Red", null, 4);
                label9.Location = new PointF(legendLeft, 60);
                label8.Location = new PointF(legendLeft, 75);
            }
        }
        ...
        public void SetAllLinks(double elbowQty, double oilTrapQty, double length, double valvelength)
        {
            foreach (Node node in addFlowPiping.Nodes)
            {
        ...
        private void DrawPipingNodesNoCaculation(string dir, SystemVRF sysItem, tmpMyNode tempNode, Node node, Node parent)
        {
            ...
            node.Location = new PointF(tempNode.Location.X, tempNode.Location.Y);
            addFlowPiping.Nodes.Add(node);
            ...
            node.FillColor = tempNode.FillColor;
            ...
            if (node is MyNode)
            {
                ...
                if (tempNode.MyInLinks != null && myNode.MyInLinks != null)
                {
                    for (int i = 0; i < tempNode.MyInLinks.Length && i < myNode.MyInLinks.Count; i++)
                    {
                        ...
                        if (myLink.AddFlow == null)
                        {
                            addFlowPiping.AddLink(myLink, parent, node);
                        }
        ...
        private MyNode LoadPipingNodeRecursively(SystemVRF sysItem, tmpMyNode tempNode, Node parent)
        {
            ...
            else if (tempNode is tmpMyNodeYP)
            {
                ...
                for (int i = 0; i < temp.MaxCount; i++)
                {
                    nodeYP.AddChildNode(LoadPipingNodeRecursively(sysItem, temp.ChildNodes[i], nodeYP), i);
                }
            ...
            else if (tempNode is tmpMyNodeIn)
            {
                ...
                RoomIndoor riItem = thisProject.RoomIndoorList.Find(ind => ind.IndoorNO == temp.RoomIndoorItem.IndoorNO);
                if (riItem == null)
                {
                    sysItem.IsUpdated = true; //如果室内机信息已经不存在，则要删除室内机节点 add by Shen Junjie on 2019/3/28
                }
        ...
        private void LoadLinkStyle(MyLink myLink, tmpMyLink tmpLink)
        {
            ...
            if (tmpLink.Points != null && tmpLink.Points.Count > 0)
            {
                myLink.Points[0] = tmpLink.Points[0];
                myLink.Points[myLink.Points.Count - 1] = tmpLink.Points[tmpLink.Points.Count - 1];
            }
            myLink.Line.Style = tmpLink.Style;//tmpLink.Line.Style;
            if (tmpLink.Points != null)
            {
            ...
            myLink.DrawColor = tmpLink.DrawColor;
        }
        ...
        {
            tmpMyNodeOut newNodeOutTemp = CreateTempNodeRecursively(sysItem, sysItem.MyPipingNodeOut) as tmpMyNodeOut;

            if (newNodeOutTemp != null)
            {
                newNodeOutTemp.PipeColor = utilPiping.colorDefault;
                newNodeOutTemp.NodeBgColor = utilPiping.colorNodeBg;
                newNodeOutTemp.BranchKitColor = utilPiping.colorYP;
                newNodeOutTemp.TextColor = utilPiping.colorText;
            }

            sysItem.MyPipingNodeOutTemp = newNodeOutTemp;
        }
        ...
        public void SaveAllPipingStructure()
        {
            foreach (SystemVRF sysItem in thisProject.SystemList)
            {
                ...
                tmpMyNodeOut originalNodeOutTemp = sysItem.MyPipingNodeOutTemp;

                tmpMyNodeOut newNodeOutTemp = CreateTempNodeRecursively(sysItem, sysItem.MyPipingNodeOut) as tmpMyNodeOut;
                if (newNodeOutTemp != null && originalNodeOutTemp != null)
                {
                    newNodeOutTemp.PipeColor = originalNodeOutTemp.PipeColor;
                    newNodeOutTemp.BranchKitColor = originalNodeOutTemp.BranchKitColor;
                    newNodeOutTemp.NodeBgColor = originalNodeOutTemp.NodeBgColor;
                }
                sysItem.MyPipingNodeOutTemp = newNodeOutTemp;
            }
        }
        ...
        public void SavePipingPlottingScaleNode(SystemVRF sysItem, MyNodePlottingScale plottingNode)
        {
            ...
            if (plottingNode != null)
            {
                ...
                tmpNode.Size = plottingNode.Size;
                tmpNode.DrawColor = plottingNode.DrawColor;
                tmpNode.FillColor = plottingNode.FillColor;
        ...
        public static void SaveBuildingImageCache(tmpNodeBgImage buildingImageNode)
        {
            ...
            {
                img = Image.FromStream(buildingImageNode.ImageStream);
        ...
        /// <summary>
        /// 保存项目时，读取户型图临时文件并附加到Project对象
        /// </summary>
        /// <param name="buildingImageNode"></param>
        /// <param name="dir"></param>
        public static void AttachAllBuildingImage(Project thisProject)
        {
        ...
        /// <param name="buildingImageNode"></param>
        /// <param name="dir"></param>
        public static void SeparateAllBuildingImage(Project thisProject, bool needCache)
        {
            if (needCache)
            {
        ...
        private void UpdateTempNodeRecursively(SystemVRF sysItem, tmpMyNode tempNode, Node node)
        {
            ...
            {
                MyNode myNode = node as MyNode;
                if (tempNode.MyInLinks != null && myNode.MyInLinks.Count <= tempNode.MyInLinks.Length)
                {
                    for (int i = 0; i < myNode.MyInLinks.Count; i++)
                    {
                        SaveLinkValue(tempNode.MyInLinks[i], myNode.MyInLinks[i]);
                    }
                }
            }
        }
        ...
        private tmpMyNode CreateTempNodeRecursively(SystemVRF sysItem, Node node)
        {
            ...
            {
                MyNodeYP nodeYP = node as MyNodeYP;
                tmpMyNodeYP tempYP = new tmpMyNodeYP(nodeYP.IsCP);
                ...
                for (int i = 0; i < nodeYP.MaxCount; i++)
                {
                    tempYP.AddChildNode(CreateTempNodeRecursively(sysItem, nodeYP.ChildNodes[i]), i);
                }
                tempNode = tempYP;
            }
            else if (node is MyNodeCH)
            ...
            else if (node is MyNodeIn)
            {
                ...
                tempIn.Location = new PointF(nodeIn.Location.X, nodeIn.Location.Y);
        ...
        private void SaveLinkValue(tmpMyLink tmpLink, MyLink myLink)
        {
            ...
            tmpLink.Style = myLink.Line.Style;
            if (myLink.AddFlow != null)
            {
                List<PointF> points = new List<PointF>();
                foreach (PointF pt in myLink.Points)
                {
                    points.Add(pt);
                }
                tmpLink.Points = points;
            }
            tmpLink.DrawColor = myLink.DrawColor;
        }
        ...
        public Node DrawBuildingImageNode(Image img, SizeF size)
        {
            if (img == null) return null;
        ...
        public Node DrawBuildingImageNode(tmpNodeBgImage tempBuildingImageNode, out Image originalImage)
        {
            originalImage = null;
        ...
        public void MoveAllItems(float dx, float dy)
        {
            foreach (Item item in addFlowPiping.Items)
            {
                ...
                else if (item is Link)
                {
                    ...
                    {
                        PointF pt1 = link.Points[i];
                        float x = pt1.X;
                        float y = pt1.Y;
                        if (dx > 0) x += dx;
                        if (dy > 0) y += dy;
                        link.Points[i] = new PointF(x, y);
                    }
                }
            }
        }
        ...
        /// <param name="node"></param>
        /// <returns></returns>
        public MyNodePlottingScale DrawPlottingScaleNode(MyNodePlottingScale node)
        {
            if (node == null)
            {
                node = new MyNodePlottingScale();
                node.Location = new PointF(10, addFlowPiping.ClientSize.Height - 50);
                node.Size = new SizeF(100, 20);
                node.FillColor = Color.Black;
                node.DrawColor = Color.Black;
                ...
                //Node label = new Node();
                //initTextNode(label, "Plotting Scale");
                //label.Font = new Font("Arial", 11f, FontStyle.Regular, GraphicsUnit.Pixel);
                //addFlowPiping.Nodes.Add(label);
                //label.Parent = node;
                //label.Location = new PointF(node.Location.X, node.Location.Y - UtilPiping.HeightForNodeText);
                //node.HighlightChildren = true;
            }

            bool isVertical = node.IsVertical;
            ShapeOrientation orientation = ShapeOrientation.so_0;
            if (isVertical)
            {
                orientation = ShapeOrientation.so_270;
                node.Size = new SizeF(20, Math.Max(10, node.Size.Height));
                node.XSizeable = false;
                node.YSizeable = true;
            }
            else
            {
                orientation = ShapeOrientation.so_0;
                node.Size = new SizeF(Math.Max(10, node.Size.Width), 20);
                node.XSizeable = true;
                node.YSizeable = false;
            }

            node.Shape = new Shape(ShapeStyle.Custom, orientation);
             ...
             *     
             */
            float w = node.Size.Width, h = node.Size.Height;
            if (isVertical)
            {
                h = node.Size.Width;
                w = node.Size.Height;
            }
            float w1 = 1, h1 = 10, h2 = 3;
            gp.AddPolygon(new PointF[] {
                    new PointF(0, h - h1),          //s1
                    new PointF(0, h),               //s2
                    new PointF(w, h),               //s3
                    new PointF(w, h - h1),          //s4
                    new PointF(w - w1, h - h1),     //s5
                    new PointF(w - w1, h - h2),     //s6
                    new PointF(w1, h - h2),         //s7
                    new PointF(w1, h - h1),         //s8
                });
            gp.CloseFigure();
            string text = node.ActualLengthString;
            if (!string.IsNullOrEmpty(text))
            {
                FontFamily ft = new FontFamily("Arial");
                StringFormat sf = new StringFormat();
                sf.Alignment = StringAlignment.Center;
                sf.LineAlignment = StringAlignment.Near;
                RectangleF rect = new RectangleF(0, 0, w, h);
                gp.AddString(text, ft, (int)FontStyle.Regular, 10f, rect, sf);
            }
            node.Shape.GraphicsPath = gp;
            ...
            if (node is MyNode)
            {
                ...
                foreach (MyLink link in myNode.MyInLinks)
                {
                    ...
                    if (scale > 0)
                    {
                        ...
                        {
                            PointF pt1 = link.Points[i - 1];
                            PointF pt2 = link.Points[i];
        ...
        public Node DrawAllShapesDemo()
        {
            ...
            for (int i = 0; i < ss.Length; i++)
            {
                ...
                node.Location = new PointF(80 * (i % 10) + 80, i / 10 * 80 + 200);
                node.Size = new SizeF(20, 20);
                node.FillColor = Color.Blue;
                node.DrawColor = Color.Green;
                ...
                addFlowPiping.Nodes.Add(label);
                label.Parent = node;
                label.Location = new PointF(node.Location.X + (node.Size.Width - label.Size.Width) / 2
        ...
        public MyNodeCH getCHUpward(Node node)
        {
            ...
            if (node.InLinks[0] != null)
            {
                if (node.InLinks[0].Org != null)
                {
                    if (node.InLinks[0].Org is MyNodeCH)
                        return node.InLinks[0].Org as MyNodeCH;
                    else
                        return getCHUpward(node.InLinks[0].Org);
                }
            }
            return null;
        }
        ...
        public bool ExistCHBoxUpward(Node node)
        {
            ...
            if (node.InLinks.Count > 0 && node.InLinks[0] != null)
            {
                Node org = node.InLinks[0].Org;
        ...
        /// </summary>
        public void CheckPipesType(MyNodeOut nodeOut, bool isHR)
        {
            if (nodeOut == null) return;
            bool hasCHBoxChild;
            CheckPipeTypesRecursively(nodeOut.ChildNode, isHR, false, out hasCHBoxChild);
        }

        /// <summary>
        /// 设置某节点下方遍历所有节点的isCoolingonly属性
        /// </summary>
        private void CheckPipeTypesRecursively(Node node, bool isHR, bool hasCHBoxParent, out bool hasCHBoxChild)
        {
            hasCHBoxChild = false;
            if (node == null) return;
            if (!(node is MyNode)) return;
            ...
            if (node is MyNodeCH)
            {
                ...
                CheckPipeTypesRecursively(nodeCH.ChildNode, isHR, true, out hasCHBoxChild);
                hasCHBoxChild = true;
            }
            else if (node is MyNodeMultiCH)
            {
                ...
                {
                    CheckPipeTypesRecursively(nodeMCH.ChildNodes[i], isHR, true, out hasCHBoxChild);
                }
                hasCHBoxChild = true;
            }
            else if (node is MyNodeIn)
            {
                MyNodeIn nodeIn = node as MyNodeIn;
                nodeIn.IsCoolingonly = isHR && !hasCHBoxParent;
            }
            else if (node is MyNodeYP)
            {
                MyNodeYP nodeYP = (node as MyNodeYP);
                for (var i = 0; i < nodeYP.MaxCount; i++)
                {
                    bool hasCHBoxChildOfYP;
                    CheckPipeTypesRecursively(nodeYP.ChildNodes[i], isHR, hasCHBoxParent, out hasCHBoxChildOfYP);
                    hasCHBoxChild = hasCHBoxChildOfYP || hasCHBoxChild;
                }

                //分歧管的isCoolingonly属性取决于其上方和其下方遍历所有节点是否存在CH-Box
                nodeYP.IsCoolingonly = isHR && !hasCHBoxParent && !hasCHBoxChild;
            }
            ...
            if (isHR)
            {
                if (hasCHBoxParent)
                {
                    myNode.PipesType = PipeCombineType.HR_L_G;
                }
                else
                {
                    if (hasCHBoxChild)
                    {
                        myNode.PipesType = PipeCombineType.HR_L_LG_HG;
                    }
                    else
                    {
                        //Cooling only
                        myNode.PipesType = PipeCombineType.HR_L_LG;
                    }
                }
            }
            else
            {
                myNode.PipesType = PipeCombineType.HP_L_G;
            }
            utilPiping.setItemDefault(node);
        }
        ...
        /// 交换室内机节点
        /// </summary>
        public void SwapNodeIn(MyNodeIn nodeIn1, MyNodeIn nodeIn2)
        {
            MyNode parent1 = nodeIn1.ParentNode;
            MyNode parent2 = nodeIn2.ParentNode;

            MyNodeIn temp = new MyNodeIn(); //解决同一个ParentNode的问题 add by Shen Junjie on 2019/1/23
            ReplaceChildNode(parent1, nodeIn1, temp);
            ReplaceChildNode(parent2, nodeIn2, nodeIn1);
            ReplaceChildNode(parent1, temp, nodeIn2);

            //修复交换indoor后，还是选中的颜色的问题。  add by Shen Junjie on 2018/3/22
            utilPiping.cancelItemHover(nodeIn2);
        }
        ...
        private decimal GetW2ForFSNSE(SystemVRF sysItem)
        {
            ...
            //For FSXNSE & FSXNPE & FSXNME System, maximun additional refrigerat charge must not exceed 6.0kg.
            if (sysItem.OutdoorItem != null)
            {
                if (W2 > 6M)
                    W2 = 6M;
        ...
        public void CreatePipingNodeStructure(SystemVRF sysItem)
        {
            ...
            bool isHR = IsHeatRecovery(sysItem);
            List<RoomIndoor> indoors = thisProject.RoomIndoorList.FindAll(ind => ind.SystemID == sysItem.Id);
            ...
            //RPI-16.0,20.0FSN3PE(-f) 有两条液管和两条气管。为每个此系列的Indoor增加一个YP
            //add by Shen Junjie on 2018/01/22
            AddYPForTwoInlinkIndoor(indoors, nodeOut);
        }

        private void AppendNodeForBinaryTree(bool isHR, Node parentNode, RoomIndoor[] indoors)
        {
            ...
            if (indoorCount > 1)
            {
                ...
                RoomIndoor[] arrPart1 = new RoomIndoor[halfCount];
                RoomIndoor[] arrPart2 = new RoomIndoor[indoorCount - halfCount];
        ...
        private void AppendNodeForNormal(bool isHR, Node parentNode, RoomIndoor[] indoors, int indoorIndex)
        {
            int indoorCount = indoors.Length;
            if (indoorCount == 0) return;
        ...
        private void AppendNodeForSchemaA(bool isHR, Node parentNode, RoomIndoor[] indoors, int indoorIndex)
        {
            int indoorCount = indoors.Length;
            if (indoorCount == 0) return;
        ...
        public void AppendNodeIn(bool isHR, Node parentNode, RoomIndoor roomIndoor, out Node newParentNode)
        {
            ...
            //parentNode 有可能是Outdoor，YP，不可能是CHBox, Multiple CH Box。Delete by Shen Junjie on 2018/01/23
        ...
        public void DeleteNode(MyNode node, bool autoPipingMode)
        {
            if (node == null) return;
            ...
            else if (parent is MyNodeCH)
            {
                ...
                if (autoPipingMode)
                {
                    DeleteNode(parentCH, autoPipingMode);
                }
            }
            else if (parent is MyNodeMultiCH)
            {
                ...
                if (parentMCH.ChildNodes.Count == 0 && autoPipingMode)
                {
                    DeleteNode(parentMCH, autoPipingMode);
                }
            }
            else if (parent is MyNodeYP)
            {
                ...
                if (autoPipingMode)
                {
                    ...
                    if (parentYP.ChildCount == 0)
                    {
                        DeleteNode(parentYP, autoPipingMode);
                    }
                    else if (parentYP.ChildCount == 1)
                    {
                        ...
                        if (brother != null)
                        {
                            ...
                            if (brother.AddFlow != null && brother.InLinks != null)  //防止报错
                            {
                                foreach (Link link in brother.InLinks)
                                {
        ...
        /// <summary>
        /// RPI-16.0,20.0FSN3PE(-f) 有两条液管和两条气管。为每个此系列的Indoor增加一个YP add by Shen Junjie on 2018/01/19
        /// </summary>
        /// <param name="indoors"></param>
        /// <param name="nodeOut"></param>
        private void AddYPForTwoInlinkIndoor(List<RoomIndoor> indoors, MyNodeOut nodeOut)
        {
        ...
        /// <summary>
        /// RPI-16.0,20.0FSN3PE(-f) 有两条液管和两条气管。为此系列的Indoor增加一个YP add by Shen Junjie on 2018/01/22
        /// </summary>
        /// <param name="indoors"></param>
        /// <param name="nodeOut"></param>
        ...
        ///// <param name="sysItem"></param>
        ///// <returns></returns>
        //private bool IsTeriaryBranchSizeUp(PipingBranchKit branchkit, SystemVRF sysItem)
        //{
        ...
        public void IsBranchKitNeedSizeUp(SystemVRF sysItem)
        {
            ...
            switch (sysItem.OutdoorItem.Type)
            {
                ...
                //add by Shen Junjie on 2018/09/26
                case "JNBBQ (Top flow, 3Ph)":    //HAPQ T3         Commercial VRF High ambient, JNBBQ
                case "CNCQ (Top flow, 3Ph)":    //Tier2 Cooling only              Commercial VRF CO, CNCQ 
                // Brazil FSNS7B/5B, FSNC7B/5B Yunxiao Lin 2019/01/05
                case "Comm. 380V VRF HP, FSNS7B":
                case "Comm. 220V VRF HP, FSNS5B":
                case "Comm. 380V VRF CO, FSNC7B":
                case "Comm. 220V VRF CO, FSNC5B":
                    break;
                default:
        ...
        /// </summary>
        private bool IsFirstPipeNeedSizeUp(PipingBranchKit branchkit, MyLink firstPipe, SystemVRF sysItem)
        {
            if (sysItem == null || !sysItem.IsInputLengthManually) return false;
            ...
            if (branchkit != null && branchkit.SizeUP == "FALSE") //注意部分机型不需要SizeUp
            {
                ...
                {
                    if (sysItem.PipeEquivalentLength >= 70d)
                        return true;
                }

                if (sysItem.PipeEquivalentLength > 100d)//一般情况下，如果MaxEqPipeLength>100m，则肯定是要SizeUp
                    return true;
        ...
        ///// </summary>
        //public bool IsHeaderBranchSizeUp(PipingBranchKit headerbranch, MyNodeYP cp, SystemVRF sysItem)
        //{
        ...
        /// </summary>
        private bool IsCHBoxSizeUp(PipingChangeOverKit changeoverkit, MyNodeCH ch)
        {
        ...
        private bool IsIndoorSizeUp(MyNodeIn indoor, string factoryCode, string regionCode)
        {
            ...
            if (isExceed15)
            {
                ...
                string[] cc = GetPipeSizeIDU_SizeUp(regionCode, factoryCode,
                    ...
                    PipingSizeUPType.TRUE.ToString());
        ...
        {
            string region = thisProject.SubRegionCode;
            string series = sysItem.Series;
            string unitType = sysItem.SelOutdoorType;

            if (thisProject.SubRegionCode == "TW") //台湾地区都用KW
            {
                return false;
            }
            ...
            if (unitType == "FSNS (Top discharge)" ||
                ...
                unitType == "Comm. 380V VRF HP, FSNS7B" ||
                unitType == "Comm. 220V VRF HP, FSNS5B" ||
                unitType == "Comm. 380V VRF CO, FSNC7B" ||
                unitType == "Comm. 220V VRF CO, FSNC5B" ||
                unitType == "FSXNME (Side discharge)" ||
                unitType == "FSVNME (Side discharge) - 1 phase" ||
                unitType == "FSNME (Side discharge) - 3 phase")
            {
                return true;
            }
            return false;
        ...
        //    foreach (MyLink lk in (node as MyNode).MyInLinks)
        //    {
        ...
        /// <param name="sysItem"></param>
        /// <returns></returns>
        public PipingBranchKit getFirstHeaderBranchPipeCalculation(MyNodeYP nodeYP, SystemVRF sysItem, out PipingErrors errorType)
        {
            ...
            PipingBranchKit branchKit = null;
            PipingBranchKit branchKitNormal = null;
            ...
            branchKit = GetHeaderBranch(type, coolingCapacity, PipingSizeUPType.FALSE.ToString(), nodeYP.ChildCount, sysItem.OutdoorItem.Type, thisProject.SubRegionCode); //20170513 增加 ODU UnitType 参数

            if (branchKit == null)
            {
                //如果FALSE查不到数据，试试NA
                branchKit = GetHeaderBranch(type, coolingCapacity, PipingSizeUPType.NA.ToString(), nodeYP.ChildCount, sysItem.OutdoorItem.Type, thisProject.SubRegionCode); //20170513 增加 ODU UnitType 参数
            }
            //判断梳形管需不需要放大一号
            if (nodeYP.NeedSizeUP)// || IsTeriaryBranchSizeUp(branchKit, sysItem))
            {
                branchKit = GetHeaderBranch(type, coolingCapacity, PipingSizeUPType.TRUE.ToString(), nodeYP.ChildCount, sysItem.OutdoorItem.Type, thisProject.SubRegionCode); //20170513 增加 ODU UnitType 参数
            }
            ...
            if (branchKit != null)
            {
                ...
                PipingBranchKit firstBranch = getFirstPipeCalculation(nodeYP, sysItem, out errorType);
                if (firstBranch != null)
                {
        ...
        public PipingBranchKit getFirstPipeCalculation(MyNode myNode, SystemVRF sysItem, out PipingErrors errorType)
        {
            errorType = PipingErrors.OK;
            ...
            {
                branchKit = GetFirstBranchKit(factoryCode, type, outItem.Type, 0, PipingSizeUPType.FALSE.ToString(), isMal, 0, outItem.RegionCode);
                if (branchKit == null)
                    branchKit = GetFirstBranchKit(factoryCode, type, outItem.Type, 0, PipingSizeUPType.NA.ToString(), isMal, 0, outItem.RegionCode);
                branchKitNormal = branchKit;
                //判断是否需要放大一号
                if (myNode.NeedSizeUP || IsFirstPipeNeedSizeUp(branchKit, myLink, sysItem))
                {
                    branchKit = GetFirstBranchKit(factoryCode, type, outItem.Type, 0, PipingSizeUPType.TRUE.ToString(), isMal, 0, outItem.RegionCode);
                }
            }
            else
            {
                ...
                //branchKit = pipBll.GetFirstBranchKit(factoryCode, type, outItem.Type, outItem.CoolingCapacity, PipingSizeUPType.FALSE.ToString(), isMal);
                //查找Firstbranch 增加IVX IDU数量的处理 20170704 by Yunxiao Lin
                int IDUNum = 0;
                int tempNum = sysItem.GetIndoorCount(thisProject);
                if (sysItem.OutdoorItem.Type == "HVN(P/C/C1)/HVRNM2 (Side flow)" && (sysItem.PipingLayoutType != PipingLayoutTypes.Normal || tempNum == 2) && tempNum != 3)
                    IDUNum = tempNum;

                branchKit = GetFirstBranchKit(factoryCode, type, outItem.Type, capacity, PipingSizeUPType.FALSE.ToString(), isMal, IDUNum, outItem.RegionCode);
                if (branchKit == null)
                    //branchKit = pipBll.GetFirstBranchKit(factoryCode, type, outItem.Type, outItem.CoolingCapacity, PipingSizeUPType.NA.ToString(), isMal);
                    branchKit = GetFirstBranchKit(factoryCode, type, outItem.Type, capacity, PipingSizeUPType.NA.ToString(), isMal, IDUNum, outItem.RegionCode);

                branchKitNormal = branchKit;
                //判断是否需要放大一号
                if (myNode.NeedSizeUP || IsFirstPipeNeedSizeUp(branchKit, myLink, sysItem))
                {
                    //branchKit = pipBll.GetFirstBranchKit(factoryCode, type, outItem.Type, outItem.CoolingCapacity, PipingSizeUPType.TRUE.ToString(), isMal);
                    branchKit = GetFirstBranchKit(factoryCode, type, outItem.Type, capacity, PipingSizeUPType.TRUE.ToString(), isMal, IDUNum, outItem.RegionCode);
                }
            }
            if (branchKit == null)
            {
        ...
        public void GetSumCalculationInversion(PipingBranchKit previousBranchKit, Node parent, Node node, SystemVRF sysItem,
           L2SizeDownRules L2SizeDownRule, out PipingErrors errorType)
        {
            ...
            if (node is MyNodeYP)
            {
                ...
                PipingBranchKit branchKit = previousBranchKit;
                PipingBranchKit branchKitNormal = branchKit;

                string type = "2Pipe";
                if (nodeYP.PipesType == PipeCombineType.HR_L_LG_HG)
                {
                    type = "3Pipe";
                }
                ...
                if (!nodeYP.IsFirstYP)
                {
                    ...
                    {
                        #region CP 梳形分歧管
                        branchKit = GetHeaderBranch(type, coolingCapacity, PipingSizeUPType.FALSE.ToString(), nodeYP.ChildCount, unitType, thisProject.SubRegionCode); //20170513 增加 ODU UnitType 参数

                        if (branchKit == null)
                        {
                            //如果FALSE查不到数据，试试NA
                            branchKit = GetHeaderBranch(type, coolingCapacity, PipingSizeUPType.NA.ToString(), nodeYP.ChildCount, unitType, thisProject.SubRegionCode); //20170513 增加 ODU UnitType 参数
                        }
                        if (branchKit == null)
                        {
                            //还查不到就抛出错误
                        ...
                        if (nodeYP.NeedSizeUP)// || IsTeriaryBranchSizeUp(branchKit, sysItem))
                        {
                            branchKit = GetHeaderBranch(type, coolingCapacity, PipingSizeUPType.TRUE.ToString(), nodeYP.ChildCount, unitType, thisProject.SubRegionCode); //20170513 增加 ODU UnitType 参数
                        }
                        #endregion
                    }
                    else
                    {
                        ...
                        int IDUNum = 0;
                        int tempNum = sysItem.GetIndoorCount(thisProject);
                        if (unitType == "HVN(P/C/C1)/HVRNM2 (Side flow)" && tempNum != 3 && sysItem.PipingLayoutType != PipingLayoutTypes.Normal)
                            IDUNum = tempNum;

                        //EU特殊处理piping使用马力值处理逻辑 add on 20180328 by Vince
                        branchKit = GetTertiaryBranchKit(factoryCode, type, unitType, coolingCapacity, PipingSizeUPType.FALSE.ToString(), isMal, IDUNum, sysItem.OutdoorItem.RegionCode);
                        if (branchKit == null)
                        {
                            //如果FALSE查不到数据，试试NA
                            branchKit = GetTertiaryBranchKit(factoryCode, type, unitType, coolingCapacity, PipingSizeUPType.NA.ToString(), isMal, IDUNum, sysItem.OutdoorItem.RegionCode);
                        }
                        ...
                        {
                            //EU特殊处理piping使用马力值处理逻辑 add on 20180328 by Vince
                            branchKit = GetTertiaryBranchKit(factoryCode, type, unitType, coolingCapacity, PipingSizeUPType.TRUE.ToString(), isMal, IDUNum, sysItem.OutdoorItem.RegionCode);
                        }
                        ...
                        if (nodeYP.PipesType == PipeCombineType.HR_L_LG)
                            specG_h = "-";
                        else
                            specG_h = branchKit.HighPressureGasPipe.Trim();
                    ...
                    specL = branchKit.LiquidPipe.Trim();
                    //Cooling Only取2pipe的分歧管了，所以不需要此处的特殊处理了  modified by Shen Junjie on 2019/3/7
                    //Heat Recovery 只有3Pipe的分歧管，如果当前配管只有2管，取到管径后需要手动设置High Pressure Gass pipe或Low Pressure Gass pipe为"-"
                    //if (isHR && branchKit.Type == "3Pipe" && nodeYP.IsCoolingonly)
                    //    specG_h = "-";
                    //else
                    ...
                    AdjustPipeSizeOfBranchKitsAfterFirstBranch(sysItem.MyPipingNodeOut, nodeYP, L2SizeDownRule);
                }
                ...
                for (int i = 0; i < nodeYP.MaxCount; i++)
                {
                    GetSumCalculationInversion(branchKit, nodeYP, nodeYP.ChildNodes[i], sysItem, L2SizeDownRule, out errorType1);
                    if (errorType == PipingErrors.OK)
                    {
                ...
                if (nodeYP.IsFirstYP)
                {
                    //子节点都循环结束后，恢复first branch的Spec_Normal  add by Shen Junjie on 2018/2/28
                    myLink.SpecL_Normal = firstPipeSizeNormal[0];
                    myLink.SpecG_h_Normal = firstPipeSizeNormal[1];
                    myLink.SpecG_l_Normal = firstPipeSizeNormal[2];
                }
                #endregion
            }
            else if (node is MyNodeMultiCH)
            {
                ...
                //某些系列的机组使用Horse Power选择合适的Multi CH Box add on 20180420 by Shen Junjie
                if (isUseHorsePower)
                {
                ...
                for (int i = 0; i < branchCount; i++)
                {
                    ...
                    GetSumCalculationInversion(null, nodeMCH, child, sysItem, L2SizeDownRule, out errorType1);
                    if (errorType == PipingErrors.OK)
                    {
                        errorType = errorType1;
                    }
                    ...
                    if (child is MyNodeIn)
                    {
                        ...
                        //当multi CH-Box分支上单独连接7HP,8HP或10HP室内机时，可以不受分支最大容量限制 20180822 by Yunxiao Lin
                        if (childHP != 7d && childHP != 8d && childHP != 10d)
                        {
                ...
                PipingMultiCHBox multiCHBox = GetMultiCHBox(thisProject.SubRegionCode, series, coolingCapacity, false,
            ...
            else if (node is MyNodeCH)
            {
                ...
                //EU特殊处理piping使用马力值处理逻辑 add on 20180328 by Vince
                PipingChangeOverKit chKit = new PipingChangeOverKit();
                PipingChangeOverKit chKitNormal = new PipingChangeOverKit();
                chKit = GetChangeOverKit(factoryCode, coolingCapacity, PipingSizeUPType.FALSE.ToString(), IUCount, series, thisProject.SubRegionCode); // 只连接一个室内机，管径不放大
                if (chKit == null) //FALSE找不到，试试NA
                {
                    chKit = GetChangeOverKit(factoryCode, coolingCapacity, PipingSizeUPType.NA.ToString(), IUCount, series, thisProject.SubRegionCode);
                }
                if (chKit == null)
                {
                    //还查不到就抛出错误
                ...
                {
                    //重新获取CH-Box管型
                    chKit = GetChangeOverKit(factoryCode, coolingCapacity, PipingSizeUPType.TRUE.ToString(), IUCount, series, thisProject.SubRegionCode);
                    if (chKit == null)
                    {
                ...
                if (nodeCH.ChildNode is MyNodeIn)
                {
                    ...
                    myLink.SpecL_Normal = myLink.SpecL;
                    GetSumCalculationInversion(null, nodeCH, nodeIn, sysItem, L2SizeDownRule, out errorType1); //Indoor如果需要放大一号，管径会变化
                    if (errorType == PipingErrors.OK)
                    {
                        errorType = errorType1;
                    }
                ...
                else if (nodeCH.ChildNode is MyNodeYP)
                {
                    ...
                    GetSumCalculationInversion(null, nodeCH, nodeYP, sysItem, L2SizeDownRule, out errorType1);
                    if (errorType == PipingErrors.OK)
                    {
                        errorType = errorType1;
                    }
            ...
            else if (node is MyNodeIn)
            {
                ...
                {
                    //重新计算Indoor的配管尺寸
                    string[] cc = GetPipeSizeIDU_SizeUp(thisProject.RegionCode, factoryCode, coolingCapacity, indoor.RoomIndooItem.IndoorItem.Model_Hitachi, PipingSizeUPType.TRUE.ToString());
                    if (cc == null)
                    {
                        //还查不到就抛出错误
                        //JCMsg.ShowWarningOK(Msg.WARNING_DATA_EXCEED);
        ...
        /// <summary>
        /// 根据区域和产品系列获取一个值，指示该如何调整first branch或者后面multi-kit的管径和型号。
        /// 部分区域的产品需要增大1st branch的管径以跟后面的管径匹配，
        /// 而另外一部分要求减小1st branch后面的其它multi-kit的管径 
        /// </summary>
        public L2SizeDownRules GetL2SizeDownRule(SystemVRF sysItem)
        {
            MyProductTypeDAL ptDAL = new MyProductTypeDAL();
            MyProductType productType = ptDAL.GetItem(thisProject.BrandCode, thisProject.SubRegionCode, sysItem.OutdoorItem.ProductType);
        ...
        /// <summary>
        /// 调整first branch或者后面multi-kit的管径和型号
        /// 部分区域的产品需要增大1st branch的管径以跟后面的管径匹配，
        /// 而另外一部分要求减小1st branch后面的其它multi-kit的管径
        /// </summary>
        /// <remarks>add by Shen Junjie 2018/2/21</remarks>
        private void AdjustPipeSizeOfBranchKitsAfterFirstBranch(MyNodeOut nodeOut, MyNodeYP currentYP, L2SizeDownRules rule)
        {
            ...
            {
                string sSizeFirst = lkFirst.GetValue<string>(p[0]);
                string sBeforeCurrent = lkCurrent.GetValue<string>(p[0]);
                string sAfterCurrent = lkCurrent.GetValue<string>(p[1]);
                double sizeFirst;
                ...
                if (rule == L2SizeDownRules.AlwaysSizeUp1stBranch)
                {
                    ...
                    {
                        //增大1st branch管径
                        lkFirst.SetValue(p[1], sAfterCurrent);
                    }
                }
                else if (rule == L2SizeDownRules.SometimesSizeDown2ndBranch)
                {
                    if (sizeBeforeCurrent == sizeAfterCurrent)  //L2 =< 40m (No Size up condition)
                    {
                        if (sizeFirst < sizeBeforeCurrent)  //Before size-up: Main pipe < After 1st branch pipe
                        {
                            ...
                            lkCurrent.SetValue(p[1], sSizeFirst);
                        }
                        else
                        {
                    ...
                    else //40m < L2 =< 90m (Size up condition)
                    {
                        ...
                        else  //Before size-up: Main pipe < After 1st branch pipe
                        {
                            if (sizeFirst < sizeAfterCurrent)  //After size-up: Main pipe < After 1st branch pipe (2 size larger than main pipe)
                            {
                                ...
                                lkFirst.SetValue(p[1], sBeforeCurrent);
                                lkCurrent.SetValue(p[1], sBeforeCurrent);
                            }
                        }
                    }
                }
            ...
            if (!currentYP.IsCP && (
                ...
                || (firstYP.PipesType == PipeCombineType.HR_L_LG_HG && currentYP.PipesType == PipeCombineType.HR_L_LG)
                ))
            {
        ...
        private void DrawPipeLableColor(Node label, string pipeSize, Node nodeUnit, int GasNo)
        {
        ...
        public void StatisticsSystem_HighDiff(SystemVRF sysItem, List<MyNode> list)
        {
            ...
            List<RoomIndoor> indoorList = new List<RoomIndoor>(); // 室内机集合 
            ...
            //统计CHBox and Indoor Units Height difference
                ...
                foreach (MyNode node in list)
                {
                    if (node is MyNodeCH)
                    {
                        ...
                        if (nodeCH.ChildNode is MyNodeIn)
                        {
                            ...
                            if (inNode.RoomIndooItem.PositionType == PipingPositionType.Lower.ToString())
                            {
        ...
        private void RoomIndoor_HighDiff(SystemVRF system, List<RoomIndoor> list)
        {
            if (list.Count > 0)
            {
                //获取 Higher 的最大值
                double HigherDiff = 0.0;
                List<RoomIndoor> listH = list.FindAll(p => p.PositionType == PipingPositionType.Upper.ToString());
                if (listH != null && listH.Count > 0)
                    HigherDiff = listH.Max(work => work.HeightDiff);

                //获取 Lower 的最大值
                double LowerDiff = 0.0;
                List<RoomIndoor> listL = list.FindAll(p => p.PositionType == PipingPositionType.Lower.ToString());
        ...
        /// <returns></returns>
        private double Calculate_HighDiff(List<RoomIndoor> list)
        {
        ...
        //            if (nodeYP.IsCP)
        //            {
        //                branchKit = pipBll.GetHeaderBranch(type, nodeYP.CoolingCapacity, PipingSizeUPType.FALSE.ToString(), nodeYP.ChildCount);

        //                if (branchKit == null)
        //                {
        //                    //如果FALSE查不到数据，试试NA
        //                    branchKit = pipBll.GetHeaderBranch(type, nodeYP.CoolingCapacity, PipingSizeUPType.NA.ToString(), nodeYP.ChildCount);
        //                }
        //                if (branchKit == null)
        //                {
        //                    //还查不到就抛出错误
        ...
        //                {
        //                    branchKit = pipBll.GetHeaderBranch(type, nodeYP.CoolingCapacity, PipingSizeUPType.TRUE.ToString(), nodeYP.ChildCount);
        //                }
        //            }
        //            //else if (HeatType == "R")
        //            //{
        //            //    branchKit = pipBll.GetTertiaryBranchKit(factoryCode,type, unitType, nodeYP.CoolingCapacity, PipingSizeUPType.FALSE.ToString());
        //            //    if (branchKit == null)
        //            //    {
        //            //        //如果FALSE查不到数据，试试NA
        //            //        branchKit = pipBll.GetTertiaryBranchKit(factoryCode, type, unitType, nodeYP.CoolingCapacity, PipingSizeUPType.NA.ToString());
        //            //    }
        //            //    if (branchKit == null)
        //            //    {
        //            //        //还查不到就抛出错误
        ...
        //            //    {
        //            //        branchKit = pipBll.GetTertiaryBranchKit(factoryCode, "3Pipe", unitType, nodeYP.CoolingCapacity, PipingSizeUPType.TRUE.ToString());
        //            //    }
        //            //}
        //            else
        //            {
        ...
        //                {
        //                    branchKit = pipBll.GetTertiaryBranchKit(factoryCode, type, unitType, 0, PipingSizeUPType.FALSE.ToString(), isMal);
        //                    if (branchKit == null)
        //                    {
        //                        //如果FALSE查不到数据，试试NA
        //                        branchKit = pipBll.GetTertiaryBranchKit(factoryCode, type, unitType, 0, PipingSizeUPType.NA.ToString(), isMal);
        //                    }
        //                    if (branchKit == null)
        //                    {
        //                        //还查不到就抛出错误
        ...
        //                    if (IsTeriaryBranchSizeUp(branchKit, nodeYP, curSystemItem))
        //                    {
        //                        branchKit = pipBll.GetTertiaryBranchKit(factoryCode, type, unitType, 0, PipingSizeUPType.TRUE.ToString(), isMal);
        //                    }
        //                }
        //                else
        //                {
        //                    branchKit = pipBll.GetTertiaryBranchKit(factoryCode, type, unitType, nodeYP.CoolingCapacity, PipingSizeUPType.FALSE.ToString(), isMal);
        //                    if (branchKit == null)
        //                    {
        //                        //如果FALSE查不到数据，试试NA
        //                        branchKit = pipBll.GetTertiaryBranchKit(factoryCode, type, unitType, nodeYP.CoolingCapacity, PipingSizeUPType.NA.ToString(), isMal);
        //                    }
        //                    if (branchKit == null)
        //                    {
        //                        //还查不到就抛出错误
        ...
        //                    if (IsTeriaryBranchSizeUp(branchKit, nodeYP, curSystemItem))
        //                    {
        //                        branchKit = pipBll.GetTertiaryBranchKit(factoryCode, type, unitType, nodeYP.CoolingCapacity, PipingSizeUPType.TRUE.ToString(), isMal);
        //                    }
        //                }
        //            }
        //        }
        ...
        //            getSumCalculation(ref n, factoryCode, type, unitType); //Indoor如果需要放大一号，管径会变化
        ...
        //            int IUCount = 1;
        //            PipingChangeOverKit chKit = pipBll.GetChangeOverKit(factoryCode, nodeCH.CoolingCapacity, PipingSizeUPType.FALSE.ToString(), IUCount, curSystemItem.OutdoorItem.Series); // 只连接一个室内机，管径不放大
        //            if (chKit == null) //FALSE找不到，试试NA
        //                chKit = pipBll.GetChangeOverKit(factoryCode, nodeCH.CoolingCapacity, PipingSizeUPType.NA.ToString(), IUCount, curSystemItem.OutdoorItem.Series);
        //            if (chKit == null)
        //            {
        //                //还查不到就抛出错误
        //                JCMsg.ShowWarningOK(Msg.WARNING_DATA_EXCEED);
        ...
        //            PipingChangeOverKit chKit = pipBll.GetChangeOverKit(factoryCode, nodeCH.CoolingCapacity, PipingSizeUPType.FALSE.ToString(), IUCount, curSystemItem.OutdoorItem.Series);
        ...
        //            {
        //                //重新获取CH-Box管型
        //                chKit = pipBll.GetChangeOverKit(factoryCode, nodeCH.CoolingCapacity, PipingSizeUPType.TRUE.ToString(), IUCount, curSystemItem.OutdoorItem.Series);
        //            }
        //            if (chKit == null)
        //            {
        //                //还查不到就抛出错误
        ...
        //        {
        //            //重新计算Indoor的配管尺寸
        //            string[] cc = pipBll.GetPipeSizeIDU_SizeUp(factoryCode, indoor.RoomIndooItem.IndoorItem.CoolingCapacity, indoor.RoomIndooItem.IndoorItem.Model_Hitachi, PipingSizeUPType.TRUE.ToString());
        //            if (cc == null)
        //            {
        //                //还查不到就抛出错误
        //                JCMsg.ShowWarningOK(Msg.WARNING_DATA_EXCEED);
        ...
        //            myLink.SpecG_l = cc[1];
        //            myLink.SpecL = cc[0];
        //            myLink.SizeUP = PipingSizeUPType.TRUE.ToString();
        //        }
        //        else
        //        {
        //            specG_h = indoor.RoomIndooItem.IndoorItem.GasPipe.Trim();
