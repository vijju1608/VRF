...
namespace JCHVRF.DAL
{
    public class OutdoorDAL : IOutdoorDAL
    {
        ...
        public OutdoorDAL(string region, string brandCode)
        {
            ...
            _mainRegion = Project.CurrentProject != null ? Project.CurrentProject.RegionCode : region;
        }
        ...
        private Outdoor DataRowToOutdoor(DataRow dr)
        {
            ...
            //如果MaxTotalPipeLength_MaxIU 为0 则取MaxTotalPipeLength
            //如果MaxMKIndoorPipeLength_MaxIU为0 则取MaxMKIndoorPipeLength  on 20181030 by xyj
            item.MaxTotalPipeLength_MaxIU = (dr["MaxTotalPipeLength_MaxIU"] is DBNull) ? item.MaxTotalPipeLength : Convert.ToDouble(dr["MaxTotalPipeLength_MaxIU"].ToString());
            item.MaxMKIndoorPipeLength_MaxIU = (dr["MaxMKIndoorPipeLength_MaxIU"] is DBNull) ? item.MaxMKIndoorPipeLength : Convert.ToDouble(dr["MaxMKIndoorPipeLength_MaxIU"].ToString());
        ...
        public DataTable GetOutdoorListStd()
        {
            ...
            if (!string.IsNullOrEmpty(stdTableName))
            {
                ...
                //string query = "SELECT DISTINCT Mid(Model,1,7) as Model, Model as ModelFull, rtrim(Model_York) as Model_York, rtrim(Model_Hitachi) as Model_Hitachi, UnitType, CoolCapacity, HeatCapacity, MaxIU, RecommendedIU, "
                string query = "SELECT DISTINCT Mid(Model,1,7) as Model, Model as ModelFull,ProductCategory,ProductType, Model_York,ProductSeries, Model_Hitachi, UnitType, CoolCapacity, HeatCapacity, MaxIU, RecommendedIU, "
                ...
                if(!string.IsNullOrEmpty(_mainRegion) && (_mainRegion == "EU_W" || _mainRegion == "EU_E" || _mainRegion == "EU_S" || _region == "TW"))
                    query += " and RegionCode Like '%/" + _region + "/%'";
                   else
                    query += " and RegionCode = '" + _region + "'";

                //+"From " + stdTableName + " Where  ProductType= '" + ProductType
                /*        + "' and DeleteFlag=1"*/
                ;

                //RegionCode like '%/" + _region + "/%'"
        ...
        public DataTable GetOutdoorListStd(string ProductType)
        {
            string stdTableName = GetStdTableName(IndoorType.Outdoor);
            if (!string.IsNullOrEmpty(stdTableName))
            {
                ...
                //20160826 by Yunxiao Lin 增加输出productType列
                //20160826 by Lingjia Qiu 增加输出Series列
                string query = "SELECT DISTINCT ProductSeries"
                        //增加输出Horsepower列 20161112 by Yunxiao Lin
                        + ",Series " + ",TypeImage, Series, ProductCategory "
                    //添加AuxModelName列  2016/12/23 by axj

                    + "From " + stdTableName + " Where  ProductCategory= '" + ProductType;
                ...
                query += " and DeleteFlag=1";
                //+ "From " + stdTableName + " where DeleteFlag=1 and ProductType";
               // '%/" + _region + "/%'";
        ...
        public DataTable GetOutdoorListStdSeries(string _subregion)
        {
            string stdTableName = GetStdTableName(IndoorType.Outdoor);
            if (!string.IsNullOrEmpty(stdTableName))
            {
                ...
                //20160826 by Yunxiao Lin 增加输出productType列
                //20160826 by Lingjia Qiu 增加输出Series列
                string query = "SELECT DISTINCT Series "
                        //增加输出Horsepower列 20161112 by Yunxiao Lin
                        + ",TypeImage "
                    //添加AuxModelName列  2016/12/23 by axj
                ...
                if (_brandCode == "Y")
                    ...
                    query += " and Model_York='-'"; 
                query = "select series from ODU_Std_UNIVERSAL where RegionCode like '%/" + _subregion + "/%'";
        ...
        /// <param name="colName"></param>
        /// <returns></returns>
        public DataTable GetOutdoorTypeBySeries( string series)
        {
            string[] columnNames = new string[] { "UnitType", "ProductType" };
            string sTableName = GetStdTableName(IndoorType.Outdoor);
            if (!string.IsNullOrEmpty(sTableName))
            {
                DataTable dt = JCBase.Utility.Util.InsertDsCachForLoad(sTableName);
        ...
        public DataTable GetOutdoorTypeBySeriesProductSeriesCategory(string series, string productSeries, string productCategory)
        {
            string[] columnNames = new string[] { "UnitType", "ProductType" };
            string sTableName = GetStdTableName(IndoorType.Outdoor);
            if (!string.IsNullOrEmpty(sTableName))
            {
                DataTable dt = JCBase.Utility.Util.InsertDsCachForLoad(sTableName);
                DataView dv = dt.DefaultView;
                string filter = " Series= '" + series + $"' and ProductSeries = '{productSeries}' and ProductCategory='{productCategory}' and Deleteflag = 1";
                if (sTableName.Contains("_UNIVERSAL")) //Universal std表的RegionCode数据和其他Std表不一样，使用"/"作为分隔。 20180809 by Yunxiao Lin
                    filter += " and RegionCode like '%/" + _region + "/%'";
                else
                    filter += " and RegionCode='" + _region + "'";
        ...
        public double CalOutdoorEstCapacity(Outdoor outItem, double Horsepower, double OutTemperature, double InTemperature, bool isHeating, SystemVRF sysItem)
        {
            ...
            double systemRatio = sysItem.Ratio; //系统配比率
        ...
        public double CalOutdoorEstCapacitynew(Outdoor outItem, double Horsepower, double OutTemperature, double InTemperature, bool isHeating, JCHVRF.Model.NextGen.SystemVRF sysItem)
        {
            string model = outItem.Model;
            string partLoadTableName = outItem.PartLoadTableName;
            if (string.IsNullOrEmpty(partLoadTableName))
        ...
        {
            if (flag == IndoorType.Outdoor && !string.IsNullOrEmpty(_mainRegion) && (_mainRegion == "EU_W" || _mainRegion == "EU_S" || _mainRegion == "EU_E" || _region == "TW"))
        ...
        public Outdoor GetYorkItemBySeries(string Model_York, string series)
        {
            ...
            if (dt != null && dt.Rows.Count > 0)
            {
                ...
                //如果MaxTotalPipeLength_MaxIU 为空 则取MaxTotalPipeLength
        ...
        public DataTable GetOutdoorPowerListBySeriesAndType(string series, string Type)
        {
            ...
            if (!string.IsNullOrEmpty(sTableName))
            {
                DataTable dt = JCBase.Utility.Util.InsertDsCachForLoad(sTableName);
                DataView dv = dt.DefaultView;
                //dv.RowFilter = " Series= '" + series + "' and UnitType='" + Type + "'  and RegionCode like '%/"+_region+"/%' and  Deleteflag = 1";
