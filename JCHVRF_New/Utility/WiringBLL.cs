...
using JCHVRF.Model;
using WiringNodeGroup = JCHVRF.Model.NextGen.WiringNodeGroup;
...
using WiringNodeOut = JCHVRF.Model.NextGen.WiringNodeOut;
using MyNodeCH = JCHVRF.Model.NextGen.MyNodeCH;
using WiringNodeCH = JCHVRF.Model.NextGen.WiringNodeCH;
using MyNodeIn = JCHVRF.Model.NextGen.MyNodeIn;
using WiringNodeIn = JCHVRF.Model.NextGen.WiringNodeIn;
using MyNodeMultiCH = JCHVRF.Model.NextGen.MyNodeMultiCH;
using WiringNodeMultiCH = JCHVRF.Model.NextGen.WiringNodeMultiCH;

namespace JCHVRF_New.Utility
{
    public class WiringBLL
    {
        private old.Project _project;
        ...
        UtilityWiring utilWiring;

        public WiringBLL(old.Project thisProj, AddFlow addFlowControllerWiring)
        {
            ...
            _imageDir = @"/Image/TypeImages/";
            utilWiring = new UtilityWiring(thisProj);
            //new Uri("pack://application:,,,/Image/TypeImages/Piping.png")
        }
        public WiringBLL(old.Project thisProj, string imageDir)
        {
            this._project = thisProj;
            isHitachi = thisProj.BrandCode == "H";
        ...
        private void InitWiringNodes(AddFlow addFlowControllerWiring)
        {
            ...
            addFlowControllerWiring.CanSelectOnMouseMove = false;

            //addFlowControllerWiring.MultiSel = false;
            addFlowControllerWiring.CanMultiSelect = false;
            ...
            addFlowControllerWiring.CanChangeOrg = false;  
            addFlowControllerWiring.CanChangeDst = false;  
            addFlowControllerWiring.CanDrawNode = false;   
            addFlowControllerWiring.CanDrawLink = false;            

            //addFlowControllerWiring.CanLabelEdit = false;
            addFlowControllerWiring.CanEditContentItem = false;

            //addFlowControllerWiring.CanSizeNode = false;
            addFlowControllerWiring.CanSizeItems = false;

            //addFlowControllerWiring.CanMoveNode = false;
            addFlowControllerWiring.CanMoveItems = false;            

            //addFlowControllerWiring.LinkCreationMode = LinkCreationMode.AllNodeArea;
        ...
        //            if (controller.Model.Trim() == "CSNET MANAGER LT" || controller.Model.Trim() == "CSNET MANAGER XT")
        //            {
        //                level = 1;
        //            }
        ...
        //    {
        //        if (sysItem.OutdoorItem != null && sysItem.ControlGroupID == controlGroup.Id)
        //        {
        ...
        //    {
        //        if (exchanger.ControlGroupID == controlGroup.Id)
        //        {
        ...
        public void CreateWiringNodeStructure(ng.SystemVRF sysItem)
        {
            ...
            bool isHeatRecovery = IsHeatRecovery(sysItem);
        ...
        private void CreateWiringNodeStructure(ng.WiringNodeOut wiringOut, ng.WiringNodeCH wiringCH, Node pipingNode, bool isHeatRecovery, bool coolingOnly)
        {
            ...
            else if (pipingNode is ng.MyNodeIn)
            {
                ...
                old.RoomIndoor riItem = nodeIn.RoomIndooItem;
        ...
        /// <returns></returns>
        public WiringNodeGroup CreateCCWiringNodeStructure(ControlGroup controlGroup)
        {
            PipingBLL pipBll = new PipingBLL(_project);
            WiringNodeGroup wiringGroup = new WiringNodeGroup();

            foreach (Controller controller in _project.ControllerList)
            {
                if (controller.ControlGroupID == controlGroup.Id)
                {
                    int level = 1;
            ...
            foreach (SystemVRF sysItem in _project.SystemListNextGen)
            {
        ...
        public bool BeforeWiringValid(ControlGroup controlGroup, out string errorMsg)
        {
            ...
            foreach (SystemVRF sysItem in _project.SystemList)
            {
                if (sysItem.ControlGroupID == controlGroup.Id)
                {
                    hasSystem = true;
                    break;
                }
        ...
        */
        public bool IsHeatRecovery(ng.SystemVRF sysItem)
        {
            return sysItem != null && sysItem.OutdoorItem != null && IsHeatRecovery(sysItem.OutdoorItem.ProductType);
        }

        public bool IsHeatRecovery(string productType)
        {
